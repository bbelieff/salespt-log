...
> **먼저 — `graft/` 폴더가 없으면 아직 준비가 안 된 것이다.**
> 그래프(`graft/`)와 설정(`.mcp.json`·`.claude/settings.json`)은 이 레포의 `.gitignore`
> (`/graft/`, `*.json`)에 걸려 **커밋되지 않는다.** 클론이나 새 워크트리에서는 한 번 준비해야
> 아래 내용이 사실이 된다:
>
> ```bash
> npm install -g @nanonets/graft   # 최초 1회
> graft init --no-global           # --no-global 을 반드시 붙인다 (없으면 홈 설정 전체를 건드린다)
> graft build                      # 무료·키 불필요, 이 레포 기준 46초
> ```
>
> **준비가 안 된 상태에서 graft 를 부르면 빈 결과가 나온다. 그때는 평소대로 grep 하고,
> 없는 근거를 지어내지 않는다.** 한 번 물어 비었으면 다시 표현만 바꿔 되묻지 말 것.
>
> **질의는 영어·코드 식별자로 한다.** 지금은 구조(tree-sitter) 그래프뿐이라 한글·개념 질의는
> 잡히지 않는다. 한글 질의를 쓰려면 `graft build --deep` 이 필요하고 그건 LLM API 키와 비용이 든다.
