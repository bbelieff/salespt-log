...
- **⚠️ 이름을 선언하기 전에 그 슬롯이 비었는지 확인한다.** `BBE-75` 하트비트 최근 코멘트를 읽어
  같은 `<머신>+<공급자><역할>` 로 **살아 있는 세션**이 있는지 본다. 있으면 그건 남의 자리다 —
  **다음 빈 글자로 옮겨** 선언하고 첫 응답에 `슬롯 <구>→<신> 변경`을 명시한다. `작업원A~F` 가
  전부 차 있으면 진행하지 말고 belie 에게 증설을 묻는다.
  (2026-08-09 사고: 데탑 `C작업원A~F` 가 전부 점유된 상태였는데 배차 프롬프트가 그걸 알려주지
  않아, 같은 날 PARKED 된 노트북 `G작업원A~F`(코덱스 6세션)와 글자가 겹쳐 관제 대시보드에서
  세션이 뭉쳐 보였다. belie 가 직접 정정 지시.)
...
## 프로덕션 UI 참조 — 2026-09-16 사용자 지시
- Muse 위임 시 수정 대상과 함께 실제 공통 컴포넌트 경로를 명시한다: `components/TopHeader.tsx`, `components/PageContainer.tsx`, `app/globals.css`, 해당 화면이 재사용하는 카드/행.
