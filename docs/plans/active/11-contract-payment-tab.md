...
|---|---|---|---|
| **A** | (공란) | — | 시스템 예약 |
| **B** | 순번 | int | 자동 (row index 또는 시트 수식) |
| **C** | 계약일 | date | **04 업체관리 자동 연동** (계약 액션 시점) |
| **D** | 업체명 | text | **04 업체관리!G 자동** |
| **E** | 수임비 | number | **04 업체관리!L 자동** |
...
| **L** | **플러그 이관** | bool (체크) | 사용자 입력 (7번째 체크박스 — 확정) |
| **M** | 수납1 진행기관 | text | 사용자 입력 |
| **N** | 수납1 현황 | text | 사용자 입력 |
| **O** | 수납1 승인금액 | number | 사용자 입력 |
| **P** | 수납1 수납액 | number | 사용자 입력 |
| **Q** | 수납1 수납일 | date | 사용자 입력 |
| **R~V** | 수납2 (5필드 동일: 진행기관/현황/승인금액/수납액/수납일) | — | 사용자 입력 |
| **W~AA** | 수납3 (5필드 동일) | — | 사용자 입력 |

**체크박스 7개**: F·G·H·I·J·K·L (서류 6 + 플러그 이관 1)
**자동 연동 3 필드**: C(계약일)·D(업체명)·E(수임비) ← 04 업체관리
...
- 2026-05-02 컬럼 매핑 확정 (A=공란, B=순번, C/D/E=자동연동, L=플러그이관 7번째 체크박스)
