...
> **📄 이 문서는 무엇인가요?**
> - **한 줄 요약**: gcal-1 — 구글 캘린더 opt-in OAuth 연결/해제 + refresh token AES-256-GCM 암호화 저장 + 연동 카드 UI(토글·캘린더 선택). 이벤트 전파는 gcal-2.
> - **누가 읽나요**: 개발자, 에이전트
...
- **레지스트리 컬럼**: users 탭 S(gcal_token 암호화)·T(gcal_settings JSON) append(A2:R→A2:T,
...
- **UI** (calendar/_components/GcalConnectCard.tsx): §4-1 문구 정본. 미연결/연결됨/실패 3상태.
  금지 용어(동기화·토큰·OAuth·만료·캘린더 ID) 화면 미노출.

## gcal-2 로 미룸 (스코프 밖)
- [다시 올리기](전체 재동기화)·이벤트 생성/수정/삭제 전파·gcal_event_ids 컬럼·gcal-client(v3 쓰기).
  → 카드에 [다시 올리기] 버튼은 gcal-2 에서 추가(전파할 이벤트가 생긴 뒤).
...
## Log
- 2026-07-09 구현: crypto+레지스트리 S/T+OAuth+service+3라우트+카드. 365 테스트 초록.
