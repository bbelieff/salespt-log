...
- **A2(아레나 시즌2) 백필 미완료**(worklog 2026-08-06 FM 기록, BBE-50 Done→Todo 원복) — A2 는
  `isDbReadPilot` 이 이미 파일럿으로 편입시키므로, 이 PR 배포 후 A2 학생은 DB 배치 경로를 타지만
  DB 에 실제 행이 없어 **미팅예정·완료·계약이 전부 0으로 보일 수 있다**(에러는 아님 — 조용히
  틀림). 백필이 끝나기 전까지는 A2 학생 화면 확인 후 머지하거나, 백필 완료를 먼저 기다릴 것.
- **§4 의 `weeklyContractsFromDb` 이월 갭**은 이 PR 미해결 — 별도 belie/FM 결정 필요.
...
- [x] check.sh 초록(typecheck·lint·구조 25/25·단위 1038·doc-drift·500줄 캡)
- [ ] §6.8 배포 확인 — **머지는 8/7 개막 이후**(belie 지시, 관리자 화면 전체 영향)
- [ ] belie: A2 백필 완료 확인 또는 A2 학생 라이브 확인 후 머지 승인
