...
- Notion은 내부 권한 사용자에게 canonical 회의록 직접 링크와 14열 복사만 제공한다. KPI 자동 쓰기와 Kakao 기능은 추가하지 않는다.
...
- [x] 회의록 확장 미리보기를 canonical Notion 직접 링크로 교체하고, 금주미팅·금주계약을 선택 금~목 DB 미팅행 상태 기준으로 정정(이월 제외, 해지·결제 미차감).
