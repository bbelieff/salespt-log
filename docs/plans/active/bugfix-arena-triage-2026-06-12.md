...
- 2026-06-14 **P14 완료**(feat/trainer-player-toggle): 수강생출신 트레이너(오민석·류서하·김종근·최정한) 트레이너 페이지 착지 + "내 아레나 일지" 토글. [라우팅] pickPreferredUser trainer 행 우선(아레나 trainee 보유해도 /trainer 착지). [연결] me.ownArenaSheetId=이름매칭 A1-N trainee sheetId(동명이인 가드, 데이터 쓰기 없음). [self-view] flag 쿠키(salespt_arena_self) + loadDashboard override sheetId(=ownArenaSheetId). [UI] TrainerPlayerToggle(트레이너 관리/내 아레나 일지) — TrainerCohortView·대시보드 양쪽, ownArenaSheetId 있을 때만. [데이터] 최정한 onjung0401 trainer 행 추가, 김종근 jkk.masterbiz 중복 trainer archived. rejoin-routing.test +2(trainer 우선). 일반 트레이너·일반 아레나 회귀 없음.
- 2026-06-14 **P15 완료**(fix/updates-toggle-bulk): 업데이트 현황 UI — 노출 체크박스→토글 스위치(파랑/회색·노출/숨김), 행별 저장 제거→상단 "변경 N건 저장" 일괄(dirty 추적·Promise.all·기준선 갱신), 변경행 노란 좌측보더+배경. append-updates.mjs 기본 노출 규칙: 사용자용 Changelog 줄 있는 feat/fix만 visible=TRUE, 내부전용·docs·chore·refactor는 FALSE(관리자 토글 override). 기존 적재분 불변(신규 수집부터). append-updates-parse.test +2.
- 2026-06-14 **P15-fix**(fix/updates-toggle-layout): 업데이트 현황 토글 레이아웃 — 행별 "노출/숨김" 글자 제거(토글 색만, aria-label "노출 여부"), 제목 input flex-1 min-w-0(긴 제목이 우측 안 밀어냄), 마일스톤·토글 고정폭 shrink-0, 열 헤더 1줄 추가. 라벨 잘림("출") 해소.
