...
- [x] PR → 직렬 머지 → §6.8(배포·health) — PR #733, squash SHA 7516ae6, 배포 run 31252481548 success,
      health `https://salesptlog.online` HTTP 200
- [x] 라이브 재확인(읽기 전용, VPS SSH로 프로덕션 Postgres 직접 조회) — 손기학 A2-4기 03 DB관리 raw:
      매입DB 2행(구매일 2026-06-19, 총 909,104 — 원 사고 수치와 일치) 모두 개강일(2026-08-07) 이전 →
      수정 로직상 100% 이월 분류, 시즌비용 0. 직접생산·현수막·콜지기소는 합계행뿐 실행 0건(활동 0
      전제 재확인). 대시보드 UI 자체(3줄 렌더링)는 admin impersonation 로그인이 필요해 이번엔
      생략(자격증명 입력 회피) — 프로덕션 DB 실측이 로직 정합성의 직접 증거. 상세: Linear BBE-83 완주 코멘트.
