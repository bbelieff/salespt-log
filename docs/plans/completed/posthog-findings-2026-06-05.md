...
- [x] **`Failed to fetch`** (`/schedule` 1건) — PR4 `chore/posthog-p2-checks`: 기존 에러 UI 에 **"다시 시도"(refetch) 버튼** 추가(새로고침 없이 재요청). 토스트 인프라 도입은 1건 빈도 대비 과한 작업 → 미도입.
- [x] **`$exception` 자동수집 — 코드 검증 완료**: `app/providers.tsx` init 에 `capture_exceptions: true` **이미 정확 설정**. 0건은 설정 문제가 아니라 "실제 JS 예외 없음"으로 해석. 운영 스모크(고의 에러 1회 → $exception 확인)는 코드로 불가 → 사람 직접. `$dead_click`은 기본 미수집이라 부재 정상.
