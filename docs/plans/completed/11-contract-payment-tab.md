...
| 그룹 | 의미 |
|---|---|
| **계약당일 받아올 것** | C/D/E 자동 연동 (미팅카드에서 계약/업체명/수임비) |
| **계약 직후 프로세스** | 6 체크박스 — 서류 진행 |
| **실무진행** | 플러그 이관 (7번째 체크박스 — 확정) |
| **수납1 / 수납2 / 수납3** | 분할 수납 3 슬롯 |

### 2행 (필드 헤더) — 컬럼 매핑 v2 (prototype v9 + 사용자 디스커버리 반영)

> **v2 변경 (2026-05-05)**: 슬롯 5컬럼→6컬럼 (진행률 추가) + 슬롯 시작 위치 변경 + 단위 만원→원

| 컬럼 | 필드 | 타입 | 출처 |
|---|---|---|---|
| **A** | (공란) | — | 시스템 예약 |
| **B** | 순번 | int | 자동 |
| **C** | 계약일 | date | **04 업체관리!D 자동** |
| **D** | 업체명 | text | **04 업체관리!G 자동** |
| **E** | 수임비 | number (원) | **04 업체관리!L 자동** |
| **F~L** | 7 체크박스 | bool | 공동인증서/임대차/신분증/드라이브/사업계획서초안/컨설팅5종/플러그이관 |
| **M~R** | 수납1 (6필드) | — | M=진행기관, **N=진행률**, O=현황, P=승인금액, Q=수납액, R=수납일 |
| **S~X** | 수납2 (6필드 동일) | — | T=진행률 |
| **Y~AD** | 수납3 (6필드 동일) | — | Z=진행률 |

**진행률 dropdown** (N/T/Z): `0% / 20% / 40% / 60% / 80% / 100%` (6단계)

**파생값 — 수수료 합계**: `Q + W + AC` (각 슬롯 5번째 = 수납액 합).

**일자 정책**: 영업관리 집계는 **수강기간(8주) 합계만**. 월별/일별 분할 X.
...
### R1. 컬럼 A~E 매핑 ✅ 확정
사용자 답변: A=공란, B=순번, C=계약일, D=업체명, E=수임비 (C/D/E는 04 업체관리 자동 연동)
...
- ✅ 시트 02 계약수납관리 layout 확정 (v2 — Drive MCP 직접 검증 완료)
- ✅ prototype v9 HTML 추가 (`docs/design/prototypes/contract-payment.html`)
- ✅ 핸드오프 자료 (`docs/handoff/contract-payment/`)

## v2 후속 PR 분할

**A (이번 PR — docs)**: prototype v9 + 핸드오프 자료 + SSOT v2 + plan v2  
**B (next)**: 백엔드 refactor — PaymentSlot 6필드 + 진행률 + 슬롯 시작 위치 변경 (M/S/Y)  
**C (next)**: UI 재구현 — prototype v9 픽셀 매칭
...
- 2026-05-02 컬럼 매핑 v1 (5컬럼 슬롯 가정)
- 2026-05-05 prototype v9 + 디스커버리 검증 → **v2 모델 확정**:
  - 슬롯 5→6컬럼 (진행률 N/T/Z 추가)
  - 슬롯 시작 M→M, R→S, W→Y
  - 단위 만원→원
  - 수수료 = Q+W+AC 파생값 (별도 컬럼 X)
  - 일자 정책: 수강기간 합계만
