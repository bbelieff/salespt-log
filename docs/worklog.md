...
### 2026-09-01 · DC(260827) · 사전등록이 안 보이던 두 번째 관문 — `if (!r[0]) continue`
- 의도: 스키마(email 빈값 허용)를 고쳐 배포했는데도 11기 7명이 여전히 명단에 없었다.
- 한 것: `lib/repo/users.ts:listAllUsers` 에 **두 번째 관문**을 발견 — `if (!r[0]) continue` (r[0]=email). 빈 이메일 행은 **파싱조차 안 하고 건너뛴다.** 스키마만 고쳐선 소용없었다(실측: 배포 후에도 63명). 조건을 「완전히 빈 행」(email·cohort·name 모두 빈값)으로 바꿨다.
- 결정: 같은 결함이 두 관문에 나뉘어 있었다 — 하나만 고치고 "고쳤다"고 하면 안 된다는 사례. 회귀 테스트 3개(빈 이메일 보존 / 완전 빈 행은 계속 버림 / 기존 행 불변).
- 다음: 배포 후 11기 7명 화면 확인 · 이정우 종강칸 2026-10-02(4주, belie 선택 A).
- SoR: lib/repo/users.ts listAllUsers 주석


### 2026-09-01 · DC(260827) · 11기 생성 + 「사전등록이 안 되는」 진짜 원인 — 빈 이메일 행이 검증에서 조용히 사라졌다
- 의도: belie "11기 명단 만들어줘" + "신규수강생 사전등록이라는게 안되지않나?".
- 한 것: 11기 7명(이진호·이효순·이정우·우정연·최석민·유성미·노경준) 개인 시트 생성 + cohorts 설정 저장 + registry 행 등록 완료. 그런데 `/admin/users` 에 안 떴다 — 서버는 "이미 등록됨"이라 답하는데 화면은 63명 그대로. 캐시·DB미러·백필 전부 배제한 뒤 원인 확정: **`User` 스키마의 `email: z.string().email()`**. 사전등록 행은 이메일이 빈 값인데(설계), 검증에 탈락하고 `parseRow` 가 실패를 null 로 삼켜 **행이 통째로 사라졌다**. 10기 김옥선도 같은 이유로 안 보였다.
- 결정: `email` 을 `z.union([z.string().email(), z.literal("")])` 로 완화. 로그인·조회는 email 매칭이라 빈 값 행은 그 경로에 자연히 안 걸린다 — 여기서 막을 이유가 없었다. 이메일이 있으면 형식 검증은 그대로. 회귀 테스트 4개(반증: 되돌리면 1개 실패).
- 다음: 배포 후 11기 7명 화면 확인 · 이정우 종강칸을 2026-10-02 로(4주 디데이, belie 선택 A).
- SoR: lib/types/user.ts email 필드 주석
