...
- 구현: fluid 컨테이너, PC 헤더 거터 통일, 1440px DB/컨택 2열·1600px 대시보드 3열, 차트 종횡비 유지 높이 제한, 데스크탑 탭바 여백 제거. Muse 실제 구독 호출 2회가 구현·검사 초안 수행. Codex는 공유 주간목표 최소 높이/여백도 학생 셸 안으로 한정하는 통합 보정.
- 실측: 실제 컴포넌트·합성 데이터로 390/1024/1280/1440/1920/2560px 전 탭 가로 넘침 없음. 1600px 경계 추가 확인. 1920×1080 대시보드 전체 요약+차트가 1080px 안에 표시됨. DB 방문 폼은 숨김 상태로 유지, 모바일 바 보존. 긴 수납 상세·목록은 자연 스크롤 유지.
- 검증 기록·실행 메타데이터: 작업 폴더 artifacts/desktop-fluid-20260925. 필수 check.sh는 pre-commit에서 실행하며 CI·운영 검증 결과는 PR/해당 산출물에 기록.


### 2026-09-25 · [병렬트랙] Codex/Muse · PC 글래스 사이드바 운영 적용
- 사용자 승인: 확정 목업 기반 구현·검증·병합·배포·운영 확인. 소유: (app) 반응형 셸·내비게이션·수납 PC 목록/상세·관련 스타일/테스트/문서. 모바일 4+1·데이터·권한·자동저장·미저장 보호 보존.
- 기존 #1018 전주 PT복사 수정 이후 ec2233b 기준 전용 worktree. 열린 별도 PR 파일 범위와 겹치지 않음.
- Muse 구현·회귀 초안, Codex 요구사항·통합·독립 검증. 실데이터를 Muse에 전달하지 않고 목업의 예시 데이터는 제품에 넣지 않음.


### 2026-09-25 · [병렬트랙] Codex/Muse · 전주 PT과제 성과 복사 매칭
- 사용자 요청: 회의록 클립보드에서 전주 과제와 해당 성과를 함께 표시.
- 소유: components/weekly-goals/copy.ts 및 관련 테스트·문서. 편집기와 pairPriorOutcomes를 공유하고 빈 성과/과제 없는 기존 성과를 보존. DB·권한·Notion 열 순서는 변경하지 않는다.
- Muse 구현·회귀 테스트 초안, Codex 통합·검증·배포·운영 복사 확인.


### 2026-09-24 · [병렬트랙] Codex/Muse · 컨택 자동저장 입력 방해 제거
- 사용자 요청: 입력마다 저장 로딩으로 느려지는 컨택관리 개선; 안전한 개선이 없으면 저장 버튼 복원.
- 원인: 자동저장 mutation도 전역 차단 오버레이 대상으로 집계하며, 성공 후 재조회에서 본문 전체 opacity가 변함.
- 소유: LoadingProvider의 명시적 백그라운드 저장 구분, contact-hooks의 호출별 선택, 컨택 페이지 연결과 관련 회귀 테스트. API·DB·권한·다른 페이지 자동저장 정책 보존.
- Muse 구현·테스트, Codex 요구사항·통합·독립 검증·배포. 합성 데이터로 지연/오류/연속 입력을 검증하고 운영 수치를 시험 변경하지 않음.
- 기존 큐·800ms 묶음 저장·실패 재시도·이탈 보호 유지. 신규 예약 등록·삭제·기록 이동 및 초기 조회는 기존 로딩 유지.

### 2026-09-24 · [병렬트랙] Codex/Muse · Drive 자동 연결 복구
- 사용자 요청: 12기 이후에도 부모 메타데이터 누락으로 자동 연결 실패가 재발하지 않도록 구현·검증·운영 반영.
- 소유: 일반 기수 drive-link 라우트, 전용 폴더 발견 helper, 연결정보 DB 저장, 관련 회귀와 ADR-0033. 기존 PR954/다른 작업 브랜치 변경 없음.
- Muse 구현·테스트 초안 및 보완, Codex 범위 판단·통합·운영 읽기 검증·배포. 실데이터는 Muse에 전달하지 않음.
- 기수 고정값·전체 드라이브 동명 폴더 검색 제거, 기존 연결 보존, DB 저장 완료 후 응답. Google 권한 변경/파일 생성 없음.
- 정정: 9/20 기록의 `parents: 0 = 코드 문제 아님`은 9/24 실제 폴더 접근 대조로 반증됨. 부모 메타 부재와 실제 미공유를 구분한다.

### 2026-09-24 · Codex/Muse · DB생산 필드 정렬
- 사용자 첨부 4채널 추가폼의 설명 문구/입력 종류에 따른 높이 불일치 수정. RowForm 공통 적용으로 편집에도 반영.
- Muse 구현, Codex 라벨 여백 보완 및 실브라우저 320/390/1024/1440px × 추가/편집 × 4채널(32조합) 확인: 가로 넘침 0, 공통 높이 모바일40px/PC33.75px.
- 필드 키·수식 순서·저장 로직 보존. 관련 자동저장 회귀 및 전체 검사를 거쳐 PR/배포 증거에 결과 기록.


### 2026-09-23 · [병렬트랙] Codex/Muse · 일반 입력 자동저장
- 사용자 승인: autosave v2 구현부터 운영 확인까지. feat/autosave-20260923, base d2164b7.
- 소유: 주간목표·컨택·DB·비용·일정·수납·업체정보·할 일 및 공지 초안의 일반 저장 UI와 안전한 자동저장. 권한/게시/계약 확정은 명시 동작 보존.
- Muse A/B/C/D 파일별 독립 구현·회귀, Codex 통합·실측·최종 배포. 다른 열린 PR의 지침/Drive/기수 생성 수정 미포함.
- 계획: docs/plans/active/autosave-20260923.md. 실데이터 시험 입력 및 백필 없음.


### 2026-09-20 · Claude Code · 오류 문구를 사람 말로 (#1007)
- 사용자: 「낫트레이너 이거 뭔말이야」. 화면이 서버 `error` 코드를 그대로 찍고 있었다.
  저장소에 27종이 있고 전부 같은 식으로 샜다. `lib/util/api-error-message.ts` 로 모아
  한국어로 바꾸고 화면 14곳에 적용. 관리자뿐 아니라 수강생이 보는 클레임·드라이브 연결·
  공지 이미지 업로드도 같은 문제였다. 머지 442579c → 배포 success → 공개 health 200.
- 11기 드라이브 폴더(재확인 2026-09-20): 6명 전원 `parents: 0`. 앱은 이미 전용 안내 패널을
  정상 표시하고 `GOOGLE_SERVICE_ACCOUNT_EMAIL` 도 운영에 설정돼 있다. **코드 문제 아님** —
  안내문 배포만 남음(`artifacts/kpi-recovery/11기_폴더공유_안내.txt`).
- 계정 정리 검증(읽기 전용): onjung 담당 잔여 0건, qwas8837 trainer/active 담당 17명,
  유성미 chaewon 단일 활성. **문병규는 활성 행이 둘**(m75431799/10기, m39881799/유보) —
  이메일이 달라 계정 2개로 보인다. 과거 이 행을 DB에서 잘못 고쳐 사고를 냈으므로
  belie 확인 전까지 손대지 않는다. 지시가 오면 시트 기준으로 처리 후 backfill.
...
- 한 것: 전체 이동 시 남는 미팅 몫 보존, 명시적 최종 저장 전 무변경, 양쪽 결과창 및 기존/신규/증감 구분, 동일 예정일시 검사.
- 검증: 로컬 fixture 브라우저 모바일/PC, 전체 check.sh 및 next build. 운영 DB·Sheets 적용과 배포는 하지 않음.
- 추가: 11:05 배포 승인, PR #948. 11:08 요청한 짧은 수정 안내·원형 ? hover/클릭 도움말 반영 및 로컬 브라우저 확인.
- 다음: 추가분 CI 통과 후 머지·배포·health 확인. 일반 저장 후 Undo/다중 요청 원자화는 이번 범위 밖.
- SoR: docs/qa/2026-09-10-contact-move.md
...
- 결정: PR #799는 CLAUDE.md §2.5 보존 대상(sheets-client.ts·db/mirror.ts)과 안 격첨 BBE-68 관찰창 조건 소멸과 무관하게 머지 가능이었음. SSH 반복 실패는 코드 문제 아님 — 재시도(rerun)로 해소, 롬백 안 함.
- 다음: #798(BBE-70, 딸린 카드) 별도 판정 필요. #884(방치 계획서 21장)·#887(SA 시트 공유 6개)·#888(R7 parity 재측정) 미착수. 워크트리 44개 중 29개 고아 디렉토리 정리 필요.
- SoR: Issue #881, PR #799

### 2026-09-07 · DC(260827) · 가이드 URL — env 우선을 되물림(코드가 유일 정본)
- 의도: 카페 전환 배포 후 실측했더니 **여전히 노션**이었다.
- 한 것: 원인 = 내가 남겨둔 「env 가 있으면 env 우선」 — VPS `.env` 의 옛 노션 주소가 조용히
  이기고 있었다. `guideUrl()` 에서 env 참조 제거 → `links.ts` GUIDE_URL 이 유일 정본. 테스트 7개.
- 결정: `NEXT_PUBLIC_*` 은 빌드타임 값이라 env 로 둬도 재배포가 필요하다 — **긴급 덮어쓰기
  수단으로서 이점 0, 낡은 값이 조용히 이기는 위험만 있었다.** 「여지를 남긴다」가 함정이 된 사례.
- 다음: 없음. VPS `.env` 의 옛 줄은 미사용이라 무해(청소는 선택).
- SoR: docs/plans/active/guide-url-to-cafe.md

### 2026-09-07 · DC(260827) · 배포 실패(빌드 OOM) — 힙 상향 대신 소스맵 생성 차단
- 의도: PR #943 배포가 빌드 단계 OOM 으로 실패. 재시도도 같은 자리 → master 배포 불가 상태.
- 한 것: 원인 = `SENTRY_AUTH_TOKEN` 없이 소스맵을 **만들어서 버리고** 있었다(업로드 X,
  hideSourceMaps 로 제공도 X). `sourcemaps.disable = !토큰` 으로 조건부 차단.
  실측: 고치기 전 2048MB 로컬 빌드 OOM → 고친 뒤 같은 조건 통과. 구조 테스트 3개로 박음.
- 결정: **힙 상향은 후보가 아니다** — 4096 은 이 레포에서 이미 반증됐다(RAM 3.8GB,
  OOM-killer → BUILD_ID 누락 silent failure, 2026-05-13). 한도를 올리면 실패가 더 조용해진다.
  그래서 한도가 아니라 **일을 줄였다**. 토큰을 넣으면 자동으로 다시 켜진다.
- 다음: 빌드는 계속 자란다. 다음에 2GB 를 또 넘기면 ①VPS swap ②러너 빌드+산출물 전송
  ③의존성 다이어트 순으로 검토. 힙 상향은 제외.
- SoR: docs/plans/active/build-oom-sourcemaps.md

### 2026-09-07 · DC(260827) · 사용 가이드 목적지를 노션 → 네이버 카페 게시판으로
- 의도: belie 「팝업의 사용방법 확인하기가 노션으로 가는데 노션이 너무 오래됐다」.
- 한 것: `guideUrl()` 목적지를 카페 게시판으로. 주소를 env(`NEXT_PUBLIC_GUIDE_URL`, VPS 전용)에서
  코드(`lib/config/links.ts` GUIDE_URL)로 내림 — NEXT_PUBLIC 은 빌드타임 값이라 env 로 둬도
  재배포가 필요했고, 바꾸려면 서버 접근이 필요해 이점이 없었다. env 우선순위는 유지(긴급 덮기).
  팝업·새소식·헤더 3곳 동시 적용. 테스트 6개.
- 결정: 처음엔 「카페는 가입 벽 + 게시판 목록」이라 노션 갱신을 권했으나 **두 전제가 다 틀렸다** —
  사용자 전원이 카페 회원이고, 게시판 자료가 노션보다 많다(belie 확인). 전제가 바뀌면 판단도 바꾼다.
- 다음: **belie 확인 필요** — 배포 후 폰에서 ①게시판이 뜨는지 ②네이버 앱으로 튕기지 않는지.
  `cafe.naver.com` 은 내 도구에서 도메인 차단이라 내가 못 본다(쿠키 무관).
  VPS `.env` 에 옛 `NEXT_PUBLIC_GUIDE_URL` 이 남아 있으면 그게 이긴다 → 그 줄 삭제 필요.
- SoR: docs/plans/active/guide-url-to-cafe.md

### 2026-09-05 · DC(260827) · 업무매뉴얼 버튼 + 같은 날 빨간 확인
- 의도: belie ①실무탭 「이전 계약업체」 버튼을 업무매뉴얼(노션) 버튼으로 교체(기능은 되살릴 수
  있게 정리) ②확인 화면에서 기록일=미팅예정일은 **빨강**으로(노랑은 못 채운 칸과 헷갈림).
- 한 것: 등록 흐름을 `PriorContractRegister.tsx` 로 통째 이관(되살리기 두 줄, 서버 무변경) →
  자리엔 노션 링크(`lib/config/links.ts` WORK_MANUAL_URL, 새 탭). 확인 화면 마크를 warn(노랑)/
  check(빨강) 둘로 분리 + 「확인 필요 · 기록일과 미팅예정일 동일」 사유 표시. 테스트 9개.
- 결정: **이미 등록된 이월 계약 데이터는 그대로 보인다**(02 행·이월 매출 카드 유지) — 버튼만 뗐다.
  빨강이어도 저장은 막지 않는다(당일 미팅은 실재).
- 다음: 없음.
- SoR: docs/plans/active/work-manual-and-samedate-check.md

### 2026-09-04 · DC(260827) · 11기가 8기로 보인 사고 — 시트 B3 미기록
- 의도: belie 「11기로 적고 들어왔는데 8기로 등록됐다, 확인하고 11기로 옮겨줘」.
- 한 것: **원인 = 시트 `01 영업관리` B3(기수의 정본) 이 템플릿의 8 그대로.** 레지스트리 B열은
  11 로 정확했다(belie 입력은 맞았다). 7명 시트 B3=11·C3=이름 교정(14칸, 수식 가드 + before 출력)
  → 관리자 [🔄 동기화]로 I~L 캐시·DB 미러 갱신(processed 160·updated 139·failed 2).
  검증: 세 곳(B열/I캐시/시트B3) 모두 11 일치, /api/admin/users cohortLabel=11.
  코드 봉합: `create-cohort-members` create 모드가 `writeProfile` 로 B3:C3 를 찍는다 + 테스트 7개.
  감사 도구 `scripts/ops/cohort-label-audit.mjs` 추가(읽기 전용).
- 결정: **아레나 라우트는 이미 같은 처리를 하고 있었다**(주석까지 동일) — 숫자 기수만 빠진
  비대칭이었다. 클레임은 이걸 못 고친다(사전등록 행에 시트가 있으면 writeProfile skip) —
  그래서 **생성 시점 도장이 유일한 방어선**이고 테스트로 박았다.
- 다음: 동기화가 못 읽은 시트 2건(8기 이용호·A2-6기 최정한) SA 접근권 확인.
  link 모드는 의도적으로 시트 미접촉 유지.
- SoR: docs/plans/active/cohort-sheet-profile-stamp.md

### 2026-09-03 · DC(260827) · 기록 옮기기 선택지를 「두 축」으로 정정 (#937 후속)
- 의도: belie 정정 — 네 선택지는 **날짜를 잘못 적은 경우 3가지 + 채널을 잘못 고른 경우 1가지**다.
  내가 처음엔 넷 다 채널·날짜를 모두 바꿀 수 있게 만들어 「무엇을 잘못했는지」가 흐려져 있었다.
- 한 것: ①날짜 고치는 셋 → **채널 잠김**, ②채널 고치는 하나 → **날짜 잠김**.
  ②는 그날 그 채널 기록을 **통째로**(수치 전부 + 대기 슬롯 + 저장된 미팅 전부) 새 채널로 옮긴다.
  팝업 옆 빈 곳 클릭 = 저장 누르기 전으로(확인·이동 팝업 함께 닫힘). check.sh 초록 · 테스트 18개.
- 결정: ②가 「얼마나」를 안 묻는 이유 = 알고 보니 다른 채널이었다면 그 자리 기록이 통째로
  다른 채널 몫이라 일부만 옮기면 반쪽이 남는다.
- 다음: 저장된 미팅 카드 진입점(3호 PR) — MeetingSlotItem 499줄 분할 선행.
- SoR: docs/plans/active/contact-record-move.md

### 2026-09-03 · DC(260827) · 저장 전 확인 화면 + 「잘못 적었어요 → 기록 옮기기」
- 의도: belie ①저장 직전에 무엇이 어디로 들어가는지 안 보인다 ②기록된 날짜·채널을 나중에 못 고친다
  (예정일시는 고칠 수 있는데 04 B열 예약일·F열 채널은 화면 어디에도 없었다).
- 한 것: 확인 화면(기록하는 날짜·채널·예약된 미팅·회사명 **같은 크기** + 그날 4지표, 버튼 [잘못 적었어요]/[저장하기])
  → 3단계 옮기기(어느 미팅 → 무엇을 4선다+⟨?⟩ → 어디로 채널·날짜, 각 단계 뒤로가기).
  새 API `POST /api/daily/move`(읽고-더하고-쓰기 서버 단일화). check.sh 초록 · 신규 테스트 14개 · next build 통과.
- 결정: **생산은 어떤 선택지에서도 안 옮긴다**(미팅과 짝이 아님, API 스키마에서 아예 제외).
  **미팅예약도 안 옮긴다** — `saveContactMetrics` 가 카드 수로 재계산(ADR-0010)하므로 카드만 옮기면 양쪽이 저절로 맞는다.
  **콜·지·기·소 유입 잠금**(ADR-0029 파생값). 다른 날짜로 옮기면 그 자리에서 **바로 저장** — 슬롯은 보는 날짜에 묶여 있어 들고 갈 수 없다.
- 다음: **저장된 미팅 카드**에서도 같은 흐름 진입(2호 PR) — MeetingSlotItem 이 499줄이라 분할이 선행돼야 한다.
  belie 확인 대기: 「같은 날짜에서 채널만 바꾸기」가 옮기는 양을 이 미팅 몫(1씩)으로 잡음 — 그날 전부여야 하면 바꾼다.
- SoR: docs/plans/active/contact-record-move.md

### 2026-09-02 · DC(260827) · 콜지기소 「아직 미팅 안 잡은 영업기회」 표시 — 통계는 한 자리도 안 건드리고
- 의도: belie "step1에서 콜지기소 접수일을 정하면 그날이 아닌경우 유입에 뜨지 않는다. 매칭될 때까지 계속 쌓여야 한다."
- 한 것: 원인은 ADR-0029 — 콜지기소 유입 = **그 날짜의 접수 건수**라 접수일이 지나면 오늘 화면에서 사라진다. 세 방식(A 화면표시 / B 매칭일로 이동 / C 매일 누적)을 숫자 예시로 제시했고 belie 가 **A** 선택. C 는 유입이 하루치 칸이라 같은 건이 날짜 수만큼 중복 계상돼 8주 누적·퍼널이 무너진다(그래서 기각). 컨택탭 콜지기소 패널에 `📌 아직 미팅 안 잡은 영업기회 N건` 줄 추가.
- 결정: **저장값(01 영업관리 F)은 ADR-0029 그대로.** 표시만 추가라 지난 통계 불변. 발굴 목록은 콜지기소 패널일 때만 조회(`useLeadCandidates(enabled)`) — 다른 채널에선 요청 0. 피커는 이미 미매칭을 접수일 무관 후보로 들고 있었으므로, 이 표시는 **이미 있던 사실을 드러내는 것**이지 새 규칙이 아니다.
- 다음: 컨택탭 팝업 2종(저장 전 확인 / 기록 날짜 옮기기) 구현 — 시안 승인 완료.
- SoR: app/(app)/contact/_lib/lead-backlog.ts 독블록


### 2026-09-02 · DC(260827) · 사전등록 표시 변경 되돌림 — 내가 「설계」를 「버그」로 잘못 봤다
- 의도: belie "원래 최초로그인 하고나서 11 이름 치고 들어와야 한명씩 잡혔었네. 되돌려줘".
- 한 것: #932·#933 revert. `email: z.string().email()` 와 `if (!r[0]) continue` 를 원상복구.
- 결정: **빈 이메일 행을 명단에서 감추는 건 버그가 아니라 설계였다.** 사전등록은 시트·행만 미리 만들어 두고, 본인이 로그인해 「기수+이름」으로 클레임할 때 비로소 명단에 잡히는 흐름이다. 내가 "등록됐는데 안 보인다"를 결함으로 단정하고 두 관문을 다 열었다 — 그 결과 관리자 명단이 63→130명이 되며 **미클레임 행 67개가 한꺼번에 노출**됐고, 11기 7명이 8기 그룹에 섞여 보이는 2차 증상까지 났다(belie 확인). 되돌리면 두 증상 모두 사라진다.
- 교훈: **"안 보인다"가 곧 "버그"는 아니다.** 운영 흐름(클레임)을 먼저 확인했어야 했다. belie 가 "사전등록이 안 되지 않나?"라고 물었을 때 그건 증상 보고였지 원인 진단이 아니었는데, 내가 그걸 진단으로 받아 코드를 고쳤다.
- 다음: 11기 7명은 시트·registry 행·기수설정 모두 준비 완료 — 각자 최초 로그인 후 「11 + 이름」 입력하면 잡힌다. 이정우 종강일 2026-10-02 은 그대로 유지(별건, 되돌리지 않음).
- SoR: 이 항목


### 2026-09-01 · DC(260827) · 「유령 계약」 발본색원 — 유령이 아니라 읽기 실패였다 + 결미담 이월 제거
- 의도: belie "결미담 이월 지워" + "유령 계약 잡아내자 원인 발본색원".
- 한 것 ①: 결미담 이월 제거. 감사로 실제 값 확인(`A2-8기 r6 2026-08-29 김해 결미담 시트 이월 / DB 이월`) 후 `clear-carryover-flag.mjs`(dry-run 기본·정확일치·멱등·되돌리기 명시)로 시트 AI/AJ + DB payload 정리. 재감사에서 3건→2건, 남은 둘은 시즌 시작(8/7) 이전이라 정상. **경로도 드러났다** — AJ 가 `prior:` 로 시작(=「이전 계약업체 등록」이 붙이는 표식)인데 AK 에는 미팅 id 가 있었다. 즉 직접등록으로 이월 행이 먼저 생기고, 나중에 미팅 계약이 `appendFromContract(dateCompanyFallback)` 로 **그 행을 갱신**했는데 C/D/E·AK 만 덮어써 **이월 딱지가 안 떨어졌다**.
- 한 것 ②: 유령 원인 확정. `grid()` 가 `.catch(() => null)` 로 **모든 읽기 실패를 빈 결과로 둔갑**시켜, 실패한 사람의 DB 계약 **전부**가 유령으로 계수됐다. 108명 연속 read 라 Sheets 쿼터(60/min)를 넘겨 매번 다른 사람이 실패 → 수치가 0→67→71→12 로 요동쳤다. 실패/빈시트를 구분하고, 실패자는 유령 계수에서 빼 별도 보고하며, 사람당 1.1초 간격을 넣었다.
- 다음: 「직접등록 이월 행이 미팅 계약으로 갱신돼도 이월이 남는」 코드 결함 수정(별건) · 페이싱 적용 후 유령 실수치 재측정.
- SoR: scripts/ops/db-contract-drift-audit.mjs grid() 독블록


### 2026-09-01 · DC(260827) · 감사 두 개 정밀화 — 시리얼 날짜 오탐 제거 + 「이월 깃발」 축 신설
- 의도: ① 팬아웃 감사 첫 실행(129명·도장 424·장부 621)에서 강구수(이태평) "국수 5만원"이 누락으로 잡혔는데 04 날짜가 시리얼(46192)이라 오탐 의심 ② belie 신고 "김현민 결미담은 이월 아닌데 이월로 체크됨" — 시트 02·04 어디에도 이월 표식이 없는데 화면은 이월.
- 한 것: 팬아웃 감사 `normDate` 에 Sheets 시리얼 변환 추가(규칙을 `db-contract-drift-audit.mjs:dateish` 와 일치 — 같은 시트를 다른 규칙으로 읽으면 두 감사가 서로 어긋난다). 46192 = 2026-06-19 확인(A2 개막 8/7 이전 = 이월권). 드리프트 감사에 **⑤ 이월(AI 구분) 깃발 불일치** 축 신설 — 시트/DB 각각의 이월 여부를 비교해 불일치 건의 계약일·업체명·양쪽 값을 상세 출력. 파일럿 화면은 DB 를 읽으므로 DB 만 이월이면 시트를 봐도 이유를 못 찾고 아레나 점수·매출에서 통째로 빠진다.
- 결정: 이월 오표시 원인은 아직 **추정**(DB만 이월). 확정 전에는 수강생 데이터를 고치지 않는다 — 감사로 먼저 재현·계수한다.
- 다음: 두 감사 재실행 → 강구수 오탐 확정 · 이월 불일치 범위(김현민 1명 vs 전반) 확정 · `02 계약수납관리` 탭이 없는 2명(이장현·황정환) 탭 생성.
- SoR: scripts/ops/db-contract-drift-audit.mjs 헤더 ⑤

### 2026-09-01 · DC(260827) · 탭이 서로 신원을 덮어쓴다 — 대리접속 쿠키 공용 + 새 탭 전환의 구조적 충돌
- 의도: belie 신고 "다른 탭에 한명이 열려있으면 다른 이름으로 접속했을때 올바르지 않은 정보가 붙는다. 지난주부터." 스크린샷 = 상단 `A2-8기 김현민` + 목록은 문병규 님 계약.
- 한 것: 원인 확정 — 대리접속 신원 `salespt_as` 는 **httpOnly 쿠키(path=/)** 라 브라우저 전체 공용인데, 전환은 `/api/admin/switch` 후 **`window.open` 새 탭**이다. 먼저 열어둔 탭도 함께 바뀐다. 헤더만 안 바뀐 이유는 `useMe` 가 staleTime 1시간 + refetchOnMount/Focus/Reconnect false 라 캐시가 남아서. **읽기보다 쓰기가 위험** — 저장도 같은 쿠키(`getWritableUserEmail`)라 낡은 탭의 저장이 다른 학생 기록에 써진다.
- 결정: 신원을 탭별로 바꾸는 건 SSR(서버 컴포넌트)이 쿠키를 읽는 구조와 충돌해 범위가 크다. 대신 **탭이 스스로 알아채고 조작을 막는다** — `/api/whoami`(세션+쿠키만, 시트·DB 0, no-store, 이메일은 지문+마스킹만) + `IdentityGuard`(마운트·포커스·30초 확인, 지문 바뀌면 전체 오버레이). 자동 새로고침은 안 한다(입력 유실). 지문을 못 받으면 안 막는다(오탐 금지).
- 다음: 근본 해결(탭별 신원 — URL 쿼리 또는 헤더 기반)은 별건. 지금은 잘못 저장되는 것만 확실히 막는다.
- SoR: components/auth/IdentityGuard.tsx 독블록
### 2026-09-01 · DC(260827) · 계약 도장은 있는데 장부에 줄이 없다 — 매출 유실 경로 차단 + 전수 감사 도구
- 의도: belie 신고 "문병규 계약이 4건인데 실무수납엔 3건. 수강생이 연락왔다." → 원인 규명 + 전수 조사 + 구멍 막기.
- 한 것: 시트 원본 대조로 확정 — `04 업체관리`에 상태=계약(케이탑전기진단 8/31 **₩1,100,000**)이 있는데 `02 계약수납관리`엔 없음. 시트·DB 양쪽에 똑같이 없어 미러 드리프트가 아니고, 기존 `db-contract-drift-audit`(02 시트↔DB)로는 못 잡는 유형. 근인은 계약 처리가 두 쓰기로 쪼개진 것: ① 미팅 patch → ② 장부 append. 옛 코드는 ② 직전에 화면 데이터를 **재조회**해서 못 찾으면 **`✓ 저장 완료`** 라는 성공 표시를 띄우고 건너뛰었고(ⓐ), 이미 계약인 카드는 ②를 아예 안 태워(`!wasAlreadyContract`) 재저장으로도 복구가 안 됐다(ⓑ).
- 결정: 판정을 순수함수 `decideContractFanout` 로 분리 — 출처는 patch **전** 값 하나(재조회 제거), 못 잡으면 **blocked**(조용한 성공 금지), **이미 계약인 카드도 태운다**(재저장 = 복구), ② 실패 시 자동 1회 재시도. 재실행 안전 근거는 `appendFromContract(dateCompanyFallback=true)` 의 기존행 갱신(BBE-53) — 이중계상 없음. 전수 감사 `scripts/ops/contract-fanout-gap-audit.mjs`(읽기 전용, 앱과 동일 짝짓기 규칙) + 워크플로 신설.
- 다음: 감사 실행해 영향 범위·유실 금액 확정 · 서버측 팬아웃 검토(탭 닫힘 창 제거) · 감사 주기 실행.
- SoR: docs/plans/active/contract-fanout-gap.md

### 2026-09-01 · DC(260827) · 캐시 워밍 회귀 자수·복구 — 대상을 줄인 게 아니라 줄이면 안 되는 걸 줄였다
- 의도: belie 재신고 "또 느려졌네". `/payment` 가 "불러오고 있어요"에서 멈춤.
- 한 것: `/api/health` 실측 — `warm.lastMs: 10`(사실상 대상 0), 배포 후 52분 경과라 reload 탓 아님. **근인은 2026-08-31 내가 넣은 좁히기**: 워밍 대상을 `!isRegistryCacheComplete(u)` 로 걸렀는데, 그 판단은 `/admin/users`(운영자 화면)만 보고 세운 것이었다. 실제로 이 캐시를 쓰는 주인은 **수강생 본인 화면**이다(`me.ts:loadMe` → `readBundle`). migrate-registry-cache 로 캐시 컬럼이 채워지자 대상이 0으로 수렴 → GRACE(30분) 지난 수강생이 콜드 fetch 를 떠안았다.
- 결정: 대상을 **활성 수강생 전원**으로 되돌리고, 쿼터는 대상이 아니라 **속도**로 지킨다 — `paced()` 시작 간격 1.5s(=40 reads/min, 한도 60의 2/3). 150명 한 바퀴 ≈ 3.75분 < 주기 20분. 좁히기 금지를 테스트 2개로 박제(주석에 사유 포함). 반증: 옛 코드로 되돌리면 25개 중 5개 실패.
- 다음: 배포 후 `/api/health` 의 `warm.lastMs` 가 다시 유의미한 값(수백 ms~수 초)인지 확인 · 수강생 화면 실측.
- SoR: lib/service/cache-warm.ts 헤더 독블록

### 2026-09-01 · DC(260827) · 아레나 드라이브 연결 결손 — 폴더를 만들 게 아니라 원래 자리를 가리킨다
- 의도: belie 신고 "8기 김현민 드라이브 연결 안 됨(The caller does not have permission)". 8·9기 전수 확인 + 직접 생성 + 수강생 시절 폴더 내용 이관 지시.
- 한 것: Drive 전수 실측 — `세일즈PT_A1_…업체관리` 40여 개 전원 존재, `세일즈PT_A2_…업체관리` **0개**. 근인은 편입 경로 이원화: 관리자 화면(`create-arena-members`)은 `createFolder` 로 만들고, ops 배치(`arena-season2-batch.mjs`)는 "업체관리"·createFolder 참조 **0건**. A2 시트는 1~8기 전부 있고 폴더만 빠졌다(2026-08-05 배치).
- 결정: **폴더 생성·이관 방식을 폐기**했다. 김현민 님 업체 2건 이동을 실제로 시도해 둘 다 `The caller does not have permission` — 업체 폴더 주인이 수강생 본인이라 운영자 계정이 못 옮긴다(belie 가 본 그 문구). 대신 `drive-link` 아레나 분기에 **③ 본인 시트 부모의 `01…` 폴더** 구제 경로 추가. 16C 폴더가 있으면 그쪽 우선이라 A1 동작 불변, 공유드라이브 전체 검색 금지(남의 폴더 오연결 방지), manual 은 id 대조라 남의 폴더 계속 거부. 반증 확인: 수정 제거 시 7개 중 2개 실패.
- 다음: ops 배치에 폴더 생성 편입(재발 방지, #913 계열) · A2-7/A2-8 registry 행 전수 점검 · A2-9 는 편입 자체가 없음(belie 확인 필요) · 진단 중 만든 빈 폴더 1개 휴지통 정리.
- SoR: docs/plans/active/arena-company-folder-gap.md
...
### 2026-09-23 자동저장 v2 구현 검증 완료 — PR #1012
- 일반 입력의 저장 버튼/전용 여백 제거, 복잡한 신규 등록·결과 확정·권한·게시·반복 적용 유지.
- Muse Spark 구독 작업으로 구현/테스트를 작성하고 통합 검증에서 지연 ACK·신규 중복 생성·충돌 덮어쓰기·분할 비용 원금 보존을 보완했다. 할 일 보완 실행은 응답 시간 제한 종료 후 남긴 산출물을 실제 React 테스트 12개로 독립 검증했다.
