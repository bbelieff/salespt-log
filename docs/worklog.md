...
3-b. [ ] 🔺**A2 배치 실행 허가** (FM 대기 중 · due 8/6) — 시트 55개 복사·명단 등재. 도구·드라이런 준비
   완료(#670), **운영 시트 쓰기 권한에 막혀 정지**. 택1: (A) 이 창에 실행 허용[권장] (B) VPS 에서
   `node scripts/ops/arena-season2-batch.mjs --season-row → --canary <이름> → --all` (C) 미루면 8/7 개막 지연.
   ※email 전환은 어느 쪽이든 **8/7 아침 `--flip-emails`** 로 분리 실행(공지와 동시).
...
8. [x] **계약 "새로 추가" DB 전환 — ✅ belie 결정(08-05) = FM 권장대로 보류.** 개강 안정화 후 재상정
   (R3-3 후속·기술명 append flip / B안 재키잉)
...
9. [ ] **과거 오염 행 리페어 — ✅ 1단계 승인(belie 08-05): 읽기전용 점검부터 진행**(FM 배차, 데이터
   변경 0건 범위). 점검 결과 보고 후 2단계(실수정 여부) 재상정 — 퍼널 계약수 해지 반영 포함
