...
| 트랙 | 역할·구역 | 현재 몸(버전) | 상태 |
|---|---|---|---|
| DC | **배차 허브(에이전트팀)** — 미배정·정체 이슈 발굴·배차·진행 감시·레인 충돌 판정·거짓완료 적발 + 필요 시 P0 직접 집행 | 경영일지 DC00(260827) | **미팅예약 달력 P0 완주(2026-08-30)** — 5번째 재발을 프로덕션 직접 계측으로 근인 확정(크롬이 `opacity:0` date input 의 showPicker 를 예외없이 무시) 후, **네이티브 달력 의존 자체를 폐기**하고 직접 그리는 팝오버로 전환(PR #897→#898, `fe0f16b`, 배포 success·health 200, belie 아이폰 확인). 회귀 가드 3건+테스트 15건 신설. 이하 이전: 트래커 이관 완료·#882 배차·온보딩 템플릿 수리. 다음 = #887 배차, 워크트리 49개 정리. |
