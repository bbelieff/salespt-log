...
| 트랙 | 역할·구역 | 현재 몸(버전) | 상태 |
|---|---|---|---|
| DC | **배차 허브(에이전트팀)** — 미배정·정체 이슈 발굴·배차·진행 감시·레인 충돌 판정·거짓완료 적발. 코드 미작성(소유 구역 없음), 산출물은 worklog·이슈 코멘트뿐 | 경영일지 DC00(260827) | **트래커 이전 실측 + GitHub 전용 보드 신설(2026-08-27)** — 경영일지 카드 미이전 확정(이관 180장은 전부 MoaWork 것). Project #2 신설·필드/라벨 구성·정체 5건(#881~#885) 등재 완료. 다음 = #882 배차, #885 는 Linear 재연결 대기. |
