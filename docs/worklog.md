...
- **🔺아레나 시즌2 (2026-08-05 발주·BBE-42·due 8/7)**: 설계도 2호 = `docs/plans/active/
  arena-season2-setup.md`(정본). 참가 = A1 전원 + 7·8기 전체, A2 = 기존 시트 복사, 시즌 8/7~9/26,
  시즌매출 0·이월 누적. 반장 = **FM(260804) 착수(2026-08-05)**. ⚠️ 시즌 SSOT 코드 = R트랙(AR-2b) 구역 —
  W0 실측 결과 **코드 미접촉(운영 데이터만)** 이라 충돌 없음. W4(8/7 공지) = belie 몫.
...
8. [ ] **계약 "새로 추가"까지 DB 기준으로 바꿀지** (R3-3 후속·기술명 append flip / B안 재키잉)
...
9. [ ] **과거에 잘못 저장된 값 고치는 스크립트 돌릴지** (오염 행 리페어 + 퍼널 계약수 해지 반영)
...
- 한 것: 배치 스크립트 `scripts/ops/arena-season2-batch.mjs` 작성 + `--plan` **드라이런 검증**(쓰기 0건, 대상 **55명** 출력 확인). 설계도 §7-6·§7-7 에 실행 절차·자율결정 등재.
- 자율결정 2건: ①**2단계 분리** — 시트 복사·A2 행은 8/5~6, **email 전환은 8/7 아침**(`--flip-emails`). 지금 옮기면 belie 공지보다 이틀 먼저 화면이 바뀐다. ②**cohorts A1 J=2026-06-12 채움**(초안 결정 뒤집음) — 비워두면 시즌 판정이 "미상=전부 통과"라 미리 만든 A2 55행이 8/6까지 전광판에 0.0 으로 노출된다. 복구=셀 비우기.
- ⛔ **블로킹**: 운영 시트 쓰기(`--season-row`·`--canary`·`--all`)가 **로컬 권한 정책에 차단**됨. 우회하지 않고 정지. → belie 승인(권한 허용) 또는 VPS 실행 중 택1 필요. 수용기준 체크는 실행 후 재개.
- 다음: 승인 즉시 W1(cohorts) → 카나리아 1명 → W3 검증 → 전체 55. **W4(8/7 공지·flip-emails)는 개막일 작업.**
...
- 의도: belie 킥오프(설계도 2호 승인). 8/7 개막 A2 셋업을 반장이 실행한다.
- **트랙 선언(§3.5)**: FM 이 A2 셋업 구역 점유 = `docs/plans/active/arena-season2-setup.md` ·
  `scripts/ops/arena-season-readiness.mjs` · (W2 예정) `scripts/ops/arena-season2-batch.mjs` ·
  운영 데이터(cohorts A2 행 · users A2 행 · A2 시트 생성). **코드(lib/·app/) 미접촉.**
- **W0 실측 결론**: ①시즌 개막일 정본 = cohorts 탭 **J열 운영 데이터**(코드 상수 아님) →
  **R트랙(AR-2b) 코드 구역과 충돌 없음** ②대상 = **55시트**(A1 40 + 7기 8 + 8기 7. 설계도 "37" 정정,
  email 없는 클레임 미완 8행 포함 — 복사만·이관 없음, 인원 중복 0) ③cohorts 에 **A2 행 없음**, A1 J 도 빈칸.
- **발견·수리**: 진단 스크립트(#657, R트랙 산출물)가 **항상 실패**하고 있었다 — env 키 계약 불일치(exit 2)
  + 존재하지 않는 탭 `영업관리`(실제 `01 영업관리`) + O1 을 문자열로 읽어 연도 소실 + 기대값 없을 때 거짓 초록.
  4건 수리 후 라이브 실행 성공(참가자 40시트 스캔). W3 검증이 이 스크립트에 의존해 선행했다.
- **설계도 정정(§0.5)**: "이월매출 셀에 합산값 쓰기" 단계 **삭제 제안** — 이월매출은 저장값이 아니라
  `계약일 < O1` 판정으로 **읽을 때 계산**된다. O1=8/7 만 세팅하면 자동 충족(셀 수기 입력은 오염 위험).
- 자율결정: A1 행 J 는 그대로 둔다(8/7 자동 전환에 지장 없음, 개막 전 표시 변경 회피). 복구=불필요.
- 다음: W1(cohorts A2 행 + J=2026-08-07) → W2(카나리아 1명 → 55시트 배치) → W3 검증. **W4 공지는 belie 몫.**
