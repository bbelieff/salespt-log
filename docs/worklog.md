...
- 결정: **워밍은 근본 수정 전까지의 워크어라운드였다** — 근본은 캐시 컬럼을 채우는 것이고, 채운 뒤엔 워밍 대상이 129→9명으로 준다. 판정 규칙은 `enrichUsersWithDates` 의 `cachedComplete` 와 동일하게 맞췄다(여기만 느슨해지면 페이지가 시트 읽는 사람을 워밍이 빠뜨려 그 사람만 느려진다). 자율결정(되돌리기: `CACHE_WARM_DISABLED=1` 재기동, 또는 revert). **관측 수단 3종 신설** — `/api/health` 의 `warm` 블록 · `VPS Logs (read-only)` 워크플로 · `cache-warm-diag`. 이번에 "못 보는 것은 못 고친다"를 값비싸게 배웠다.
- 다음: **#913 아레나 기수 편입 시 캐시 컬럼 자동 충전**(안 하면 다음 시즌 그대로 재발 — 이번 근인) · **#914 운영 앱 로그가 pm2 에 하나도 안 남는 문제**(이번 추적을 막은 진짜 병목, npm 배너 3줄만 반복되고 Next 기동 배너·`console.log` 전무).
- SoR: 이슈 #903(완주 종료)·#913·#914, PR #904~#912, `lib/service/cache-warm.ts`, `scripts/ops/cache-warm-diag.mjs`, `.github/workflows/vps-logs.yml`


### 2026-08-30 · C(260820) · #882 레인 개별상세 프리페치 완주 — 서버측 warmBundle, 필드측정은 정직 보류
- 의도: belie 직배차, A 설계서 보강분("목록→학생클릭→피드백" 개별 상세 진입). B의 admin/users Suspense(레인 ②) 완주 후 순서대로 진행.
- 한 것: 설계서가 제안한 클라이언트 router.prefetch+queryClient.prefetchQuery는 실제 클릭 핸들러(`AdminUserPicker.tsx` `pick()`, `TrainerCohortView.tsx`)가 `window.open`으로 새 탭을 여는 구조라 효과가 없음을 발견(새 탭=별개 QueryClient 인스턴스) — 서버측 `warmBundle(spreadsheetId)`(신규, `profile-bundle-cache.ts`)로 방향 전환: `/api/admin/switch`가 대상 확정 직후 await 없이 호출, 새 탭의 `/api/me`가 도착할 즈음 이미 캐시가 데워져 있거나 기존 in-flight dedup을 공유. 단위테스트 4건(자기검증 완료). PR #893(`a8b2251`) 머지·배포·health 200. 실측 도구로 `slo-loop.mjs`에 9.개별상세 구간 추가(PR #901, `5869bc6`) — admin switch 직후 /api/me vs switch 없이 /api/me(콜드 대조군) 자연실험.
- 결정: **필드측정(n=3, run 33283188574) 결과는 신뢰 불가로 정직 보류** — trial마다 다른 real 학생을 써서(재사용 불가) between-subject 시트크기 잡음이 warmBundle 효과보다 커 보임(trial1: 47ms vs 1535ms 극적개선, trial3: 1013ms vs 37ms 역전). curl 합성 head-start(수십ms)가 실제 브라우저 새탭 스핀업(수백ms)보다 훨씬 짧아 이 측정 자체가 실제 효과를 과소평가하는 방향으로 편향돼 있음도 명시. **1차 증거는 단위테스트+in-flight dedup 메커니즘 근거**로 삼고, 필드측정 숫자를 성공/실패로 스핀하지 않는다(§0.8).
- 다음: 신규 배정 대기. n=3 필드측정을 신뢰성 있게 개선(trial 수 확대 또는 인위적 지연 삽입)은 후속 카드 후보로만 남김(이번 스코프는 overengineering 판단).
- SoR: `docs/plans/completed/admin-detail-warm-bundle.md`, #882·#889 코멘트, PR #893·#901

### 2026-08-30 · DC(260827) · 미팅예약 달력 5번째 재발 종결 — 네이티브 date picker 의존 자체를 제거
- 의도: belie 직접 신고("수강생들 미팅예약 카드에 달력이 또 클릭이 안돼, 모바일·PC 둘 다 되게"). 전 수강생 영향 P0. 같은 자리가 #654·#656·#730·BBE-81 에 이어 다섯 번째.
- 한 것: **프로덕션에서 직접 재현·계측**(실사용 10기 계정, 컨택관리→미팅예약 카드, 실제 마우스 클릭). 1차 근인 확정 — `showPicker()` 가 호출되고 **예외 없이 `ok` 반환하는데 달력이 안 뜬다**(`activeElement`=BODY, 폴백 미발동). 같은 클릭에서 `opacity` 만 0→1 로 바꾸면 열림 ⇒ **크롬이 `opacity:0` 인 date input 의 showPicker() 를 조용히 무시**. 우리 코드는 #730(8/07) 이후 무변경 — **브라우저가 바뀐 것**이라 앞선 세 수정이 전부 이 지점을 비껴갔다. → PR **#897**(`26c47fe`, opacity:1 + 내용만 투명화 + CSS 계약 가드 4건) 머지·배포 success·health 200. **그런데 belie 실측 결과 모바일 여전히 실패** → 접근 전환. PR **#898**(`fe0f16b`) — `DateInputCustom` 을 **직접 그리는 달력 팝오버**로 전면 교체(6주 42칸 고정·달 이동·오늘·닫기·min/max·Esc·바깥클릭·키보드, `position:fixed` 좌표계산으로 부모 overflow 회피 및 모바일 화면밖 방지, 날짜는 문자열로만 다뤄 KST 하루밀림 차단). props 계약 불변이라 호출부(MeetingSlotCard ×2·MeetingSlotItem) 무변경. 옛 `.hidden-native-date` 전량 제거. check.sh green(구조 38·단위 1423·doc-drift), 배포 run 33264749517 success·health 200.
- 결정: **네이티브 date picker 의존을 폐기한다.** 네 번의 원인이 모두 달랐지만 공통점은 하나 — **달력을 여는 주체가 브라우저**라 통제도 실패감지도 불가능했고, showPicker 는 실패해도 예외를 안 던져 폴백조차 못 건다. 부수효과가 더 중요하다: **네이티브 팝업은 페이지 밖이라 스크린샷·키입력이 안 통해 "떴는지"를 기계가 볼 수 없었다** — jsdom 테스트가 계속 초록인 채 세 번을 놓친 진짜 이유다. 직접 그리면 그 검증이 가능해진다. 실제로 배포본에서 달력 표시 + **날짜 변경(2026-08-30→2026-09-03)** 까지 실측했고, **belie 아이폰 확인 완료(8/30)** 로 완주. 자율결정(되돌리기: `git revert fe0f16b`, opacity 건까지면 `26c47fe` 도 함께).
- 다음: 인앱 브라우저(카카오·인스타)는 미확인 — 구조상 다를 이유 없으나 기수 온보딩 때 확인 권고. **워크트리 49개(권장 12)** 경고 지속 — 별도 정리 필요. GitHub 카드 **#896 완주 종료**.
- SoR: 이슈 #896, PR #897·#898, `components/ui/DateInputCustom.tsx` 상단 주석(원인 계보), `docs/design/components.md` §2, `tests/components/date-input-picker.test.ts`(15건 · 회귀 가드 3건)


### 2026-08-27 · B(260820) · #882 레인 ② Suspense 3단계 완주 — 명단 즉시 표시(구 BBE-249)
- 의도: belie 배차("A 설계서 ①「어드민 목록 Suspense 3단계」" — A 설계서 자체의 §① 섹션 번호, #882 카드의 3분할 중 ②느림 파트만). admin/users 가 force-dynamic 블로킹 SSR 하나라 8주 통계까지 다 기다려야 명단이 뜨는 문제 해소.
- 한 것: `app/admin/users/page.tsx` 를 Tier0(정적 골격, `_components/UsersSkeleton.tsx`)·Tier1(<1초 목표, DB fast-path 명단만, `UsersRoster.tsx`)·Tier2(느릴 수 있는 개인 시트 폴백 경유 통계, `UsersStatsFill.tsx`)로 분리. `AdminUserPicker`·`enrichUsersWithDates`/`Stats` 완전 무변경(재배치만). 설계서 §①-2 의 "카드별 개별 스트리밍"은 `pMapBundle(concurrency=8)` Sheets quota 가드를 깨서 배치 단위 스트리밍으로 의도적 단순화(#882 코멘트에 근거 기록). PR **#894**(`bb82c69`→squash `8e81929`) 머지·배포 run 33073791581 success·health 200(직접 curl 재확인). check.sh 초록(typecheck/lint/structural 38·unit 1453/build exit0).
- 결정: **스코프를 belie 원 배차대로 ②만** 처리 — #882 흡수 카드(구 BBE-244 크래시·프리페치③)는 착수 전 발견해 손대지 않고 #882 코멘트에 스코프 정정으로 남김(별도 배차 필요). 콜드 배포직후 실측(`slo-loop.mjs`)은 이 세션에 VPS 접속정보가 없고 belie 가 "A 담당"으로 명시해 직접 하지 않음 — before 수치(median 5,634/max 10,906ms)만 카드에 이미 있음.
- 다음: A 가 배포직후 창에서 `node scripts/ops/slo-loop.mjs` 6.어드민수강생관리 2회 연속 재실측(§882 완주기준). belie/DC00 은 ①크래시 재현여부 별도 판단.
- SoR: #882·#889 코멘트, PR #894

### 2026-08-27 · DC(260827) · 트래커 이관 집행 완료 — 경영일지 미완료 13장 전량 GitHub 이전, Linear 는 아카이브로 격하
- 의도: 앞 항목(미이전 확정)의 후속. belie 가 Linear MCP 를 재연결해줘서 이관을 실제 집행. **연결 검증부터**: 워크스페이스 `bbelieff` · 팀 `Bbelieff`(key BBE, 팀 1개) · 프로젝트 3개(경영일지 In Progress + MoaWork 2개) — 계정 정상. **이관 누락 근인 확정**: 팀이 하나인데 프로젝트가 3개라 이관 스크립트가 MoaWork 2개만 퍼갔다. BBE-75 본문에 이 함정이 이미 경고돼 있었다("두 프로젝트 접두어가 둘 다 BBE- 라 잘못 올리면 상대 관제판이 오염된다 — 2026-08-09 실제 사고, 최근 20건 중 17건이 모아워크").
- 한 것: 경영일지 카드 **71장 중 미완료 13장을 전량 처리**(잔여 0). 신규 카드 3장 — **#887**(BBE-80 · 10기 시트 6개 SA(masterbot) 공유 누락, 퇴출 후 재클레임 시 "시트 못 찾음" 실제 고객장애. 2026-08-27 재확인 = `lib/repo/drive-sa-share.ts:38` 조용한 catch 그대로, 수정 PR 0 → **아직 유효**, EVAL 0/6→6/6) · **#888**(BBE-66·133·134 통합 — R7 parity 잔재. 2026-08-14 이후 BBE-120/67/252 Done·B21 근인확정(#848·#850)·6기 잔차 근인=시트 SUMIFS 캐시 유령값(#860·#862)·시트독립 종료(#871)로 **전제가 뒤집혔을 가능성**이 커서, 첫 작업을 「고치기」가 아니라 「재측정 후 판정」으로 바꿔 이전 — 좀비 카드 방지) · **#889**(BBE-75 하트비트 대체, 이슈 상단 고정. Linear 를 끊으면 세션이 상태 남길 곳이 사라지므로 신설). 흡수 3건 — #881←BBE-69·68·70 / #882←BBE-244·249·48 / #884←BBE-37. 구조 전환 2건 — BBE-73 도장규정은 카드가 아니라 **보드 README + `완주증거` 필드**로 기계화(증거 없으면 「완주」로 못 옮김), BBE-35 belie결정함은 **라벨+Status 옵션**으로 분산(한 카드에 모으면 항목이 서로 가려짐). **원본 Linear 14장 전부에 「GitHub #n 으로 이전」 코멘트** — 추적선 유지(§D 함정 준수).
- 결정: **#883 철회·종료(자율결정)** — 내가 근거로 쓴 「일지저장 max 2,379ms」가 2026-08-23 값이었고, 2026-08-27 처방1 실측에서 이미 **max 1,847ms 로 SLO 통과**(원본 BBE-242 도 그날 05:16 Done). belie 가 고를 것이 없어져 결정 요청을 거둬들였다. **#882 를 `/admin/users` 레인 단일 창구로 확정** — 같은 화면에 미완료 카드가 3장(BBE-244 화면크래시 P0 · BBE-249 N+1 · BBE-48 원조) 동시에 살아 있어 각각 배차하면 §3.5 레인 충돌. **크래시(BBE-244)를 느림보다 먼저** 보되 근인이 같은 콜드버스트일 가능성이 높아 한 세션이 같이 본다. **BBE-68 이 여전히 Backlog** 임을 원본에서 확인 — #799 가 16일째 기다린 조건이 실제로 안 왔고, 2026-08-24 CLAUDE.md §2.5(`DB_READ_COHORTS` 존치)로 **"안 없앤다"로 뒤집혔을 가능성**까지 확인. 되돌리기: 이슈 reopen · 보드 `gh project close 2 --owner bbelieff` · 코드 변경 0.
- 다음: **Linear 연결 불요**(운영에 필요한 것 전부 GitHub·레포로 이전 완료, 아카이브로만 남김). 배차 순서 = **#882**(belie 「즉각즉각」 최우선, 크래시 우선) → #887(실제 고객장애) → #881(게이트 재판정) → #888(재측정) → #884(문서전용). 모든 세션은 **#889 에 하트비트**를 남긴다.
- SoR: GitHub Project 2, 이슈 #881·#882·#884·#887·#888·#889, 종료된 #883·#885, Linear BBE-* 이전 코멘트 14건


### 2026-08-27 · DC(260827) · 트래커 이전 실측 — 경영일지 카드 「미이전」 확정 + GitHub 전용 보드 신설
- 의도: 신규 DC(배차 허브) 세션 기동. 온보딩 전제("2026-08-27부로 Linear→GitHub Issues(bbelieff/salespt-log) 이전 완료")를 착수 전에 실측 확인 — 특히 하트비트(BBE-75)·도장(BBE-73) 규정이 GitHub 어디로 옮겨졌는지.
- 한 것: **실측 결과 전제가 틀렸다.** `salespt-log` 이슈는 총 10장·전부 4개월 전(최신 #16)으로 이관 흔적 0건. 이관본 180장은 전부 `bbelieff/moawork` 에 `[MIGRATED][BBE-*]` CLOSED 로 존재. 이관된 BBE 번호는 5~34·44·46~47·86~94·101~199·205~241·255~275 이고 **빠진 구간(35~43·45·48~85·95~100·200~204·242~254)이 정확히 경영일지 카드 구간** — Linear BBE 팀에 공존하던 두 프로젝트 중 **MoaWork 것만 퍼갔다**(worklog L318 과 정합). 하트비트 BBE-75 → 미이전(`moawork#381` 은 제목 자체가 「[BBE-94] 모아워크 세션 하트비트 — **BBE-75 아님**」인 별개 카드). 도장 규정 BBE-73 → 미이전, 접근 가능한 유일 사본 = 이 파일 L319 「공통·도장 규정 v4.1」. belie 지시로 **경영일지 전용 GitHub Project 신설**(`/users/bbelieff/projects/2`, MoaWork 보드 #1 과 분리 — `salespt-log` 에는 이 보드만 링크됨). Status 를 하네스 어휘(대기/진행중/PR 오픈/배포확인중/완주/블로킹/belie결정)로 재정의 + 필드 3종 신설(트랙·레인(소유구역)·**완주증거**) + README 에 "완주로 옮기려면 완주증거(PR·SHA·run id·health 200) 필수, 문서만 쓴 건 완주 아님" 명시(§0.8 거짓완료 차단을 보드에 기계화). 레포 라벨 6종 신설(사무소:클로드코드/코덱스/Cowork·belie결정·거짓완료의심·문서전용). 현재 정체 5건을 EVAL(측정·현재·목표·금지) 박아 카드로 등재: **#881**(PR #799 16일 정체, 게이트 BBE-68 관찰창이 BBE-245 종료로 무효화됐는지 재판정) · **#882**(어드민 수강생관리 배포직후 median 5,634ms/max 10,906ms → 명단 즉시표시) · **#883**(일지저장 max 2,379ms 잔존, belie 선택) · **#884**(active 계획서 42장 중 21장이 30일+ 방치, 최대 86일) · **#885**(옛 카드 미이전 종결).
- 결정: **경영일지의 살아있는 트래커는 아직 Linear** — 옛 BBE 번호로 새 카드를 내지 않는다. 새 카드는 `salespt-log` 이슈(#881~)로 내고 보드 #2 에 올린다. PR 번호와 같은 줄을 쓰므로 BBE 번호와 충돌 없음. moawork 의 `[MIGRATED]` 180장은 **MoaWork 것이고 경영일지 것이 아니다** — 이관본 본문도 "작업을 재개하라는 지시가 아님"을 명시하므로 이걸 근거로 작업하지 않는다. 자율결정(되돌리기: 보드는 `gh project close 2 --owner bbelieff`, 이슈는 close, 라벨은 `gh label delete` — 코드 변경 0).
- 다음: #882(belie 원문 「즉각즉각」 최우선)부터 작업원 배차. #885 는 Linear MCP 재연결(belie) 대기 — 연결되면 미완료 카드만 보드로 이관하고 원본에 "GitHub #n 으로 이전" 코멘트를 남긴다.
- SoR: GitHub Project 2 README, 이슈 #881~#885, 이 항목


### 2026-08-27 · C(260820) · BBE-242 처방2 완주 — PostHog init 을 window load 이후로 지연
- 의도: belie 집행 배차(로딩보고서 처방2) — `app/providers.tsx`의 `posthog.init()` 즉시실행을 GA 방식(화면 다 뜬 뒤)으로 전환. 실측(A): PostHog 하위 스크립트 5개가 초기 로딩과 리소스 경쟁(각 0.7~2.3초).
- 한 것: `posthog.init()`을 `window load` 이벤트(또는 이미 로딩완료 상태면 즉시)로 지연. `lib/analytics/index.ts`의 기존 "init 전 호출은 no-op" 구조가 지연로딩 시 운영에서도 이벤트를 조용히 드롭하는 걸 발견 — init 대기 중 호출을 버퍼링(`runOrQueue`)했다가 `loaded` 콜백에서 FIFO flush 하도록 전 export 교체(이벤트 유실 0). `vitest.config.ts`에 tsconfig 대비 누락돼 있던 `@/analytics` 별칭 추가(신규 테스트 작성 중 발견). 단위테스트 4건(자기검증: 버퍼링 되돌려 실패 확인 후 원복). PR **#879**(`4f5cee1`) 머지·배포 success·health 200.
- 결정: 프로덕션 직접측정(before/after, 2회 재현) — PostHog 스크립트 시작 시점이 `loadEventEnd` 이전(970ms, 로딩 중 겹침)에서 `loadEventEnd` 시점/직후(1046ms·398ms)로 이동 확인. `loadEventEnd` 절대값(1659→1046ms)은 네트워크 변동 커서 단정 안 함 — 핵심 증거는 "겹침 제거" 자체.
- 다음: E 의 처방3(전광판 SWR)·4(gcTime, #878 이미 머지 확인)·5(문서정정, 이미 머지 확인) 병행 완료된 것으로 보임 — belie/A 취합 대기.
- SoR: `docs/plans/completed/bbe242-posthog-lazy-load.md`, BBE-242 코멘트, PR #879

### 2026-08-27 · A(260820) · 로딩보고서 처방1 — 전광판 콜드 실측, 「진짜1위」가설 반증 + 어드민즉시오픈 설계서
- 의도: belie 발주 로딩보고서(E캐시절+B인프라절+A취합) 채택 → 처방1(전광판 콜드 비용 실측, 처방3 SWR이식 전 baseline). 이어서 belie 추가발주(설계대행) — 「어드민/수강생관리 즉시오픈」구조 설계서 + 목록→학생클릭→피드백 개별상세 경로 보강.
- 한 것: `slo-loop.mjs`에 8번째 경로(`/admin/arena/scoreboard`) 추가(PR #875) → 배포직후(최악조건) 실측 — **전광판 median159/max328ms(n=5/5 완전정상)**, 같은 run에서 어드민은 median5634/max10906ms(n=2/5) 그대로 재현. **E의 "전광판=105엔트리 동시만료로 진짜1위" 가설을 실측으로 반증** — 처방3(SWR이식) 우선순위 하향 권고. 설계서: admin/users 콜드버스트의 진짜 근인이 sales-DB전환과 무관한 별개 서브시스템(readBundle 프로필번들, registry캐시컬럼 I~L이 "수동버튼 동기화"라 대부분 비어있어 매번 시트폴백)임을 코드추적으로 신규 확정 — Suspense 3단계(Tier0골격/Tier1명단즉시/Tier2통계지연) 설계 + 데이터우선순위표 + 작업분할(B=admin/users최우선, C·D=admin·trainer병렬) + 완주판정기준(첫표시<1초+완전로드≤2초) 게시. 개별상세(카드클릭→impersonation전환)는 이미 client-fetch 구조라 구조 변경 불요, 프리페치만 제안.
- 결정: 전광판(처방3)은 belie/E 판단으로 스킵·후순위 가능. admin/users Suspense 전환(B레인)이 belie 원문("즉각즉각") 대상 최우선 — 구현 착수는 디스패치 라우팅 대기(설계만, 구현 금지 지시 준수).
- 다음: E·C 의 로딩보고서 처방2~4(PostHog지연로딩·gcTime상향, 처방5는 PR #876 로 이미 완주) 머지 완료 대기 → 효과 재실측(전/후 비교표) → 최종판정. 설계서 라우팅 후 B/C/D 착수 대기.
- SoR: BBE-242 코멘트(처방1 결과·설계서 2건), PR #875(scoreboard route), VPS run 33039925994(headSha 8644150 기준)

### 2026-08-23 · A(260820) · BBE-242 SLO 루프 최종 라운드 — 전 기수 DB 전환 완료 후 실측, 완주는 보류(일지저장 max 잔존)
- 의도: 6기 전환(D, 06:26 KST)까지 완료돼 전 기수 DB 정본 상태 도달 → 최종 라운드(7경로 병렬 2회, 30분+ 간격) → 종료조건("2회 연속 전경로 p95≤2초+오류0") 판정 → 최종 전/후 총표로 BBE-242 완주 지시.
- 한 것: 1회차(21:34Z, D 배포로부터 8분) — 6경로 클린, 어드민 전량실패(예상된 배포직후 콜드버스트), 일지저장 max 초과. 2회차(22:03Z, 29분 뒤) — **어드민 완전 정상화(186ms) 자연회복 실측 확정**, 나머지 5경로도 클린, **일지저장만 1·2회차 공통으로 max 초과**(2686ms→2379ms, median 은 둘 다 SLO 통과). 최종 전/후 총표 게시: 어드민 10,845ms→186ms, 전환기수(7/10) Sheets-path 대비 일지목록 최대 **46배** 개선(8,623ms→186ms) 확정.
- 결정: **종료조건 엄밀 미충족 — 완주 도장 보류**(§0.8 거짓완료 금지). 일지저장 max 스파이크만 사이클1부터 한 번도 완전히 안 없어진 진짜 잔존(median 은 이미 해결). belie 체감 문제의 근본원인(비파일럿 Sheets-path 지연)은 명백히 해소됐으나, 형식적 종료조건은 이 한 경로 때문에 미달 — belie 에게 ①사이클4(readProductions 등 잔여 Sheets 호출 추적) vs ②핵심미션 완료로 간주하고 회귀감시 모드 전환, 둘 중 선택 요청.
- 다음: belie/감시자 판단 대기.
- SoR: BBE-242 코멘트(최종 전/후 총표 전문), VPS run 32667938618·32669404925(headSha b1983e2 무렵 기준)

### 2026-08-24 · B(260820) · 6기 잔차 종결 — 근인=시트 SUMIFS 캐시의 유령값, GO 판정
- 의도: belie 종결배차("6기 잔차, 이관 금지·단독소유로 끝까지"). C의 재현결과(meetings 원행 시트=DB 완전 일치, 백필가설 반증)를 입력으로 채택하고 1명 완전분해로 근인을 file:line 까지 확정하라는 지시.
- 한 것: `f8dd7ee33216` 1명을 시트원값→시트수식경로→DB원행→DB산식경로 4단으로 완전 대조(신규 임시 스크립트, SSH 1회 다회 실행, 커밋 안 함 — 순수 조사). 04(미팅) 탭 A2:L 전수 확인 = 실데이터 7행뿐, 상태=계약 정확히 2건(330,000·550,000, DB와 완전 일치). `01 영업관리` C열 day-anchor 체인(`C10=$O$1`,`C14=C10+1`...) FORMULA 렌더로 직접 검증 — 정상(Track D의 "6기 legacy+57일 주차분기" 가설 기각). 그런데 시트 O열(SUMIFS 캐시, 대시보드 B21 이 참조하는 그 값) 은 3,630,000 — 차이 2,750,000=550,000×5, row8 하나의 금액이 캐시에서 6번(진짜1+유령5) 카운트된 것과 정확히 일치. `1a9bad13`(04탭 전체공백, 실데이터 0건)으로 재현 — 시트캐시는 5,400,000을 표시.
- 결정: **근인 = 시트 자신의 SUMIFS 캐시가 이미 삭제된 과거 미팅데이터의 유령값을 아직도 표시**(구글시트가 비활성 시트의 수식결과를 무기한 캐시하는 현상으로 추정) — DB/코드 버그가 아니라 **DB 쪽이 오히려 현재 진실을 정확히 반영**. "애매하면 시트값" 원칙의 전제(시트가 신뢰가능)가 깨짐을 직접 증명. **판정: 6기 GO**(코드수정 불요 — PR #860·#862 계산이 이미 옳음). belie 투명고지: 전환 시 일부 학생 화면숫자 하락(유령값→진짜값) 가능 — 트레이너 사전안내 권장.
- 다음: D 자동전환 대기(6기 GO 도장 완료). 6·7·10·4기 전부 판정 완료 시 ⑤ 시트=백업 선언문·BBE-245 종료는 D 소관.
- SoR: BBE-252 Linear 코멘트(근인확정+판정), BBE-75. 코드 변경 0(읽기전용 조사만, 임시 스크립트는 VPS에 남기지 않음).

### 2026-08-24 · C(260820) · BBE-252 계보 6기 meetings 백필 집행 — 진단 정정(백필 문제 아님, 실측 재현 안 됨)
- 의도: belie 집행배차("6기 meetings 백필, 시트독립 마지막 블로커") — B/D 의 19:06/19:21 코멘트 근인("한 사용자 meetings 0건 SQL 직접확인")을 전제로, 10기 개별교정 패턴을 6기 전원으로 일반화 집행.
- 한 것: `bbe252-cohort-meeting-backfill.mjs`(`--cohort` 인자, 10기 스크립트의 id공란필터·계약여부체크박스오탐수정·payload·upsert 로직 재사용) + 전용 workflow 신규(PR #867, `52c4f4c`, 배포 success·health 200). dry-run(cohort=6) 실행(run 32662620038) = **후보 0건**(6명 전원) — execute 불필요(넣을 게 없음). 곧바로 `dashboard-parity --cohort "6"` 재실행(run 32662683896)·`bbe252-cohort-readiness`(run 32662804486) 교차검증 = **meetings 원행 지문 100% 일치**(6명 전원 sheet-only/db-only 0건), 샘플 행수도 시트=DB 정확히 일치(17=17·5=5·7=7).
- 결정: **belie 배차의 전제("meetings 백필 완결성 문제")가 지금 실측으로 재현 안 됨** — meetings raw row 는 이미 시트=DB. 잔차 39건(5/6명)은 전부 집계값(N.주계약·H.주활동·B21) 레벨이고 원행은 일치 → week bucketing/courseStart 해석 문제로 추정(6기 legacy +57일 모델 특이점 의심, 코드 직접 대조는 스코프 밖). **완료 도장 보류**(BBE-252/BBE-75 둘 다) — D 자동스위치 미발동. B 에게 이관(dashboard-aggregates.ts/week.ts 영역, PR #860·#862 담당).
- 다음: B 의 week-bucketing 조사 대기. 새로 만든 cohort 백필 도구는 폐기 안 함(0건을 0건으로 정확히 보고했음 — 실제 id공란 직접입력형 잔차가 있는 기수에 재사용 가능).
- SoR: `docs/plans/active/bbe252-6gi-meeting-backfill.md`, BBE-252 코멘트, PR #867, VPS run 32662620038·32662683896·32662804486

### 2026-08-23 · B(260820) · 6기 소스 정렬 집행 — 공식 검증(1/6 diff0), meetings 백필 완결성이 새 관문
- 의도: belie 집행배차(BBE-252 마지막 관문) — "6기 집계 소스 정렬 후속, 이전 라운드 제외했던 그 작업". feature flag/기수한정 + 파일럿 회귀 0 필수 + dashboard-parity 6기 diff 0 확인 후 도장.
- 한 것: `weeklyFeeFromMeetings`(04 미팅.L, 상태="계약", 1~8주 버킷) 신규 + `computeDbAggregates`에 `feeSource{cohort,spreadsheetId}` 선택 파라미터 — `SOURCE_ALIGNED_FEE_COHORTS=Set(["6"])`만 새 경로, 생략하면 기존 계약(02) 기준 그대로(파일럿·7기 등 회귀 0, 단위테스트로 고정). 6기 legacy O5(`'02 계약관리'!D2`, 헤더존 고정 셀) 6명 1회 SSH 스냅샷 → 코드 상수(1명만 660,000). PR #860(`b0b850e`) 머지·배포·health 200. 파일럿(8·9·연습) 재확인 15명 중 14 diff0(B21 전원 0, 무관 기존 이슈 1건 그대로) — 회귀 0 실증. 재검증 중 진단도구 자체 버그(`normalizeDbMeeting` 이 수임비 필드를 아예 안 뽑고 있었음, weeklyFeeFromMeetings 추가 시 같이 안 고침) 발견·수정 PR #862(`6698252`).
- 결정: 최종 재검증(SSH, `dashboard-parity.mjs --cohort "6"`) = 6명 중 1명(068699408a10) diff 0 확정 — **공식(04 미팅.L 기준) 자체는 맞다**는 결정적 증거. 나머지 5명 잔차는 소스정렬과 무관 — 직접 SQL 대조로 1명은 DB meetings 0건(백필 자체가 안 됨) 확인, 이건 B21뿐 아니라 카드 최초부터 있던 N.주N계약·H.주N활동 잔차와 같은 근원(6기 meetings 백필 완결성 문제). "공식을 더 고친다"로는 안 풀림 — 별도 후속카드 필요. **6기 여전히 HOLD**(코드는 기수한정이라 6기가 DB_READ_COHORTS 밖인 지금 라이브 영향 0).
- 다음: 신규 배정 대기. 후속카드 후보(belie/반장 판단) = 6기 meetings(04) 시트 vs DB 백필 완결성 조사(BBE-260 식 건별 대조 — 계약 축은 이미 끝냈으니 미팅 축).
- SoR: BBE-252 Linear 코멘트(구현+재검증+최종판정 3건), PR #860·#862, VPS run(SHA `6698252` 기준)

### 2026-08-23 · A(260820) · BBE-242 SLO 루프 사이클 3 — 4·7·10기 전환 효과 실측 확정(before/after 최대40배→오차범위)
- 의도: 사이클2 제안(BBE-252 연동) 채택 확인 → D 가 4·7·10기 전환 완료(19:00Z). 전환 기수 계정 경로 개선폭 정량화 + 회귀감시 + 잔존 원인분류 + 종료조건 판정.
- 한 것: slo-loop.mjs 에 migrated identity(7/10기) 추가 + 대표계정 전환(PR #863), 측정순서 재배치로 사이클2 순서오염(일지저장이 어드민 콜드버스트 뒤에 있어 median 3배 뻥튀기) 제거(PR #865) — 머지·배포·재실측(2회, 19:20·19:22). **결과: 6/7경로 2회 연속 완전클린**(일지저장도 사이클2 오염값 4674ms→정상 1057~1135ms 회복). 결정적 증거 — 진단축에서 migrated(7/10) 와 DB(연습) 이 사실상 동일한 지연을 보임(예: 계약목록 566ms vs 763ms, 대시보드 516ms vs 462ms) — **사이클2 에서 같은 7/10기가 Sheets-path 였을 때 최대 40배 느렸던 것(일지목록 8623ms)이 지금은 오차범위로 수렴**, before/after 를 같은 스크립트·같은 계정으로 직접 증명.
- 결정: 어드민수강생관리만 배포직후 콜드버스트 회복 중(전량실패→median1387/max2594) — 사이클1 에서 이미 근인 확정(SWR캐시가 PM2 reload 로 비워짐, Redis금지 제약과 충돌해 완치 안 함, 구조적 트레이드오프로 수용). 종료조건("다른 시각 2회 연속 전경로")은 6/7경로 기준 충족, 어드민 포함 완전충족은 belie/감시자 판단 대기(6기 전환까지 묶을지, 지금 별도 확인할지).
- 다음: 잔여 6기 표본(email 유효 계정 못 찾음, 원인 미확인) 조사 + 6기 전환(B 소스정렬 완료, D 집행 대기) 후 최종 라운드. 부수관측: 짧은 간격 반복 dispatch 가 DB pool(EMAXCONNSESSION)에 압박 신호 — 사이클 간 간격 권고.
- SoR: BBE-242 코멘트(사이클3 전문, before/after 대조표), PR #863·#865, VPS run 32660882163·32661008372(headSha ea28727 기준)

### 2026-08-23 · A(260820) · BBE-242 SLO 루프 사이클 2 — 병렬측정 인프라 완성, 진짜 근인은 비파일럿 Sheets-path 구조적 지연
- 의도: 감시자 판정(사이클1 완주 확인) → 사이클2 재투입. 지시: 7경로 측정을 서브에이전트 병렬로 전환해 측정시간 단축 + 잔여 순위표 갱신 + 1위 원인 최소수정.
- 한 것: 지시된 "경로당 Agent 서브에이전트"는 인증쿠키(특히 어드민=ADMIN_EMAILS 실제계정) 노출면 확대 우려로 **VPS 신뢰 프로세스 내부 `Promise.all`**로 대체 구현(§0.5, BBE-242에 편차 고지). PR #856(경로간 병렬화)·#857(비파일럿 표본쿼리 버그 2종 수정 — cohort접미사+**email공란**, 어드민을 병렬배치에서 격리) 순차 머지·배포·재실측. 격리 후 어드민 자기경합은 해소됐으나, 진단축이 처음으로 진짜 비파일럿 계정을 찾아내면서 **DB-path vs Sheets-path 격차를 정량 확정**: 계약목록 1.1~1.4x·미팅목록 2~3.5x·대시보드 4.3x·일지목록 13~40x(local 2524ms~public 8623ms, 후자는 직전 어드민콜드버스트 자기오염 가능성 있어 절대치는 유보). VPS load 0.10~0.49(낮음) — 인프라 아님, Sheets API 자체 지연 확정. 부수발견: 어드민 격리측정(콜드버스트) 직후 곧바로 측정한 일지저장이 사이클1 대비 대폭 악화(1429ms→median4674ms) — 어드민이 소진한 Sheets 쿼터의 꼬리를 물려받은 것으로 판단(회귀 아님, 측정순서 오염).
- 결정: 이번 발견은 코드 1건 버그가 아니라 **비파일럿 기수의 구조적 Sheets 지연**이라 최소수정 PR 로 건드릴 지점이 없음 — 진짜 처방은 BBE-252(기수별 DB 파일럿 전환)와 동일 작업이라 중복시도 안 함(§0.5). SLO 루프를 ①파일럿 경로 회귀감시 ②BBE-252 전환 시마다 그 기수 재측정·검증 역할로 전환 제안 — belie/감시자 확정 대기.
- 다음: 사이클3 — 어드민 격리측정과 본측정(일지저장) 사이 간격/별도 SSH 분리로 측정순서 오염 제거, 방향 확정되면 그에 따름.
- SoR: BBE-242 코멘트(사이클2 전문, DB-path/Sheets-path 대조표), PR #856·#857, VPS run 32652608135·32653151755(headSha cf0b1e9 기준)

### 2026-08-23 · A(260820) · BBE-242 SLO 루프 사이클 1 — 실측 인프라 구축 + 일지저장 2.7초→1.4초 수정
- 의도: belie 직접 발주 상시배차(BBE-242 연장, 신규카드 불가) — "접속 불안정, 로딩 2초 이하 기준 세우고 루프 돌려라". SLO=주요 7경로 p95≤2000ms, 측정법 고정(경로당 curl 5회 TTFB/총시간 중앙값+최대).
- 한 것: 신규 `scripts/ops/slo-loop.mjs`(PR #853) — next-auth/jwt `encode()`로 세션쿠키 직접 발급(OAuth 불요), 7경로 실측 + DB-path/Sheets-path 진단축. 유일한 쓰기=일지저장 멱등 라운드트립("연습" 계정 한정). run 1 실측 결과 1위=어드민수강생관리(median 10.8초, 4/5 타임아웃) — 재실측(5분 뒤)으로 반증: median 151ms 로 정상화, `lib/service/profile-bundle-cache.ts`(BBE-249 SWR캐시)가 PM2 reload(=매 배포)에 비워지는 "배포직후 콜드버스트"였음을 확정(구조적 트레이드오프, Redis금지 제약과 충돌해 완치 안 함). 진짜 1위=일지저장(POST) median 2680ms — 근인 확정: `persistSalesRows`(sales-write.ts:187)가 채널마다(최대4회) `readCourseStart`(O1 시트read)를 매번 새로 호출(`courseDateDbReadEnabled` 게이트 OFF라 파일럿도 영향). 최소수정(PR #854, `isWithinSalesWindow`에 선택적 `precomputedCourseStart` 인자 추가, 하위호환) → 루프 밖 1회 조회로 축약 → 머지·배포·재실측: median 1429ms(SLO 통과). max 3079ms(1건 실패)는 잔존.
- 결정: 계약목록(max 2771ms)은 기존에 이미 문서화된 구조적 잔존(계약·DB관리 목록조회 상시 시트병행read, union백필 설계)이라 이번 최소수정 루프 범위 밖 — 재설계 여부는 belie 판단 필요.
- 다음: 사이클 2 — ①일지저장 잔존 스파이크 원인(`syncDirectProductionForDate`→`readProductions` 용의, 미확인) ②진단축 비파일럿 표본쿼리 버그 수정(cohort 컬럼 접미사 추정, 매번 "없음") ③종료조건(서로 다른 시각 2회 연속 전경로 달성) 미충족이라 루프 계속.
- SoR: BBE-242 코멘트(run1/2/3 수치+근인+수정 전문), PR #853·#854, VPS run 32648506264·32648756639·32649412579(headSha e13fd49 기준)

### 2026-08-20 · B(260820) · B21/주차집계 드리프트 근인 확정 — 미팅.L 기준 vs 계약.E 전체합, STATS_WEEKS 클램프 부재
- 의도: belie 집행배차(BBE-252 계보) — "네가 발견한 B21/주차집계 재계산 드리프트 해소, 이번 라운드 내 종결, 6·7기 전환의 마지막 관문".
- 한 것: SSH `valueRenderOption:FORMULA` 직접 대조로 시트 B21 진짜 정의 확정 — `대시보드!B21`=`'01 영업관리'!O4`(1~8주 stride 8셀 합)이고 그 안은 `SUMIFS('04 업체관리'!L:L,...,J:J="계약")`(미팅 L열=수임비 기준). `lib/config/index.ts:212-213`의 "계약 액션 시 D/G/L→C/D/E 자동연동" 주석과 정합 — 계약 성사 순간 1회성 복사일 뿐 이후 02(계약) 테이블과 독립적으로 벌어질 수 있음. 우리 `arenaFeeFromDb`는 02 테이블 전체합(STATS_WEEKS 8주 클램프도 누락)이라 시트와 소스·창 둘 다 달랐다. 안전한 클램프만 정렬(PR #848 프로덕션·#850 진단도구, 소스 자체 전환은 파일럿 공유함수 회귀위험 있어 보류) → 머지·배포·health 200. 재검증: 7기 a808ed0d0385(-5,000,000 diff) → 0 확정, 파일럿 15명 중 14 diff0(B21 전원 0, 회귀 없음).
- 결정: 6·7기 게이트는 기수 단위 전원동시전환이라 개인별 부분GO 불가 — 7기 5/8 클린(8/8 아님)·6기 0/6(소스자체가 다름) 둘 다 이번 라운드 HOLD 유지, belie 명시 지시 있으면 즉시 집행 가능하도록 근거만 게시.
- 다음: 신규 배정 대기. 후속카드 후보(belie/반장 판단) = ①6기 미팅.L 기반 소스전환(파일럿 회귀 검증 설계 포함) ②7기 잔차 3명 개별조사(10기 13f5c9c2 식).
- SoR: BBE-252 Linear 코멘트(근인+재검증+파일럿회귀확인 3건), PR #848·#850, VPS run(a51e43825a78f9f800d8195429db9f3f261cd9bd 기준)

### 2026-08-21 · C(260820) · BBE-252 계보 부분완주 — 10기 13f5c9c2 sheet-only 미팅 8/9건 백필 성공, 1건+B21 잔존
- 의도: BBE-252 계보 집행 배차(belie 「이번 라운드 내 종결」 확정) — read-only 조사(전 항목)에서
  확정한 처방(개별 교정)을 실제 집행. 10기 학생 1명의 sheet-only 미팅 5건(원행 9건)을 DB 백필
  → parity diff 0 확인 → D 가 10기 스위치 자동 전환.
- 한 것: 신규 `scripts/ops/bbe252-10gi-meeting-backfill.mjs`(id 공란 행만 대상, row_key=r{행}
  합성키, 공란은 키 생략으로 유지 — dashboard-parity 정규화 함수가 양쪽을 동일하게 처리함을
  실측해 기본값 치환 불필요 확인) + 전용 workflow(PR #847). **dry-run #1 에서 후보 160건 실측
  → 152건이 계약여부(K) 체크박스 컬럼의 서식 기본값(빈 행도 UNFORMATTED_VALUE 로 리터럴
  false 반환)을 오판한 가짜 후보임을 발견** — 즉시 fix(PR #849, K는 true 일 때만 데이터로
  카운트). dry-run #2 = 정확히 8건(기존 read-only 조사 9개 원행 중 id 없는 8개와 일치) →
  execute(run 32438404432) 8건 upsert 성공 → `dashboard-parity --cohort "10"` 재검증(run
  32438441642): **meetings 원행 지문 차집합에서 백필한 8건 전부 사라짐(성공 확인)**. 남은 diff
  7건은 기존에 id 를 갖고 있어 이번 백필 대상에서 애초 제외됐던 **08-17 현수막 1건**(시트=
  "계약금 들어올예정"(비표준 텍스트) vs DB="계약"/계약여부=true, 상태 불일치)이 R1:U6.현수막
  4필드+N.주2계약+H.주2활동 6건을 견인하는 것으로 보이고(단정 아님), B21(1건, 1,000,000)은
  contract 지문 자체가 sheet/db 완전 일치(0건)라 이 미팅 건과 무관한 별개 문제로 보임.
  **참고**: 병행해서 B 가 B21 STATS_WEEKS(8) 클램프 버그를 2건 수정(PR #848 앱 레벨·#850
  dashboard-parity-lib.mjs 레벨) — 내 parity 재검증(02:03 실행)이 #850 배포 완료(02:06) **이전에
  실행돼 그 수정을 반영 못 했을 가능성** 있음(타이밍 근접 실측 확인) — B21 은 그대로 B 에게
  이관, 재검증은 B 가 배포 완료 후 새로 판단.
- 결정: **08-17 현수막 1건은 기계적 백필로 못 고친다** — 이미 id 가 있어 "추가"가 아니라
  "DB 값을 시트로 되돌릴지, 시트값을 무시할지"를 사람이 정해야 하는 실데이터 판단(CLAUDE.md
  §0.7 화이트리스트 ①) — 이 세션이 임의 집행하지 않음. **diff 0 미달성이라 10기 GO 판정·
  D 스위치 전환 조건 미충족** — 완료 도장을 안 찍고 정직 보고(부분 완주).
- 다음: 08-17 건은 belie/트레이너 확인 필요(양쪽 값 중 무엇이 사실인지). B21 은 B 에게 이관
  (Linear 코멘트에 이름으로 명시). revert 필요 시 row_key `r4,r5,r6,r7,r8,r9,r10,r11`(spreadsheet_id·
  tab='meetings') 삭제로 원복.
- SoR: PR #847·#849, run 32437349170·32438133147(배포), run 32438404432(execute)·32438441642
  (parity 재검증), `docs/plans/active/bbe252-10gi-meeting-backfill.md`

### 2026-08-20 · B(260820) · BBE-260 완주 — 6기 「DB-extra 계약」은 진단도구 버그, 중복적재·삭제미반영 둘 다 0건
- 의도: BBE-252 후속. 6기 전원·7기 절반이 B21 지문대조에서 "DB에만 있는 계약"으로 잡힌 원인을 (a)중복적재/(b)시트삭제-미반영 건별로 판정.
- 한 것: BBE-258 방식 심층해부 스크립트(`bbe260-contract-drift-deepdive.mjs`) 신규 작성 → 1차 SSH 실행에서 6기 6/6 전원 `Unable to parse range: '02 계약수납관리'!A6:AK` 확인(탭 자체를 못 읽음). `lib/repo/contract-payment.ts TAB_ALIASES`(6기 legacy `02 계약관리`/5행) 미적용이 원인 — 정본 측정기 `dashboard-parity.mjs`도 동일 버그 보유(BBE-252 run 32389628088의 "6기 6/6 불일치"가 이 버그의 산물이었음). 두 파일 모두 alias-fallback 추가(PR #842·#843·#844) → 2차 SSH 재실행: 6기·7기 14명 전원 계약 지문(계약일+수임비+구분) 기준 onlyInDb 0건·countMismatch 0건 — **(a)/(b) 둘 다 아니다.**
- 결정: 카드가 물은 "정리 방법 설계"(②③)는 해당없음으로 종결 — 정리할 실데이터가 없다. 후속카드 권장(별도, 이 카드 범위 밖): 6기 5/6명이 계약 레코드는 일치하는데 B21 합계·주차별계약수는 여전히 diff — legacy 시트 수식 자체의 이월판정/계산 방식이 현행과 다를 가능성.
- 다음: 신규 배정 대기.
- SoR: BBE-260 Linear 코멘트(결정판), PR #842·#843·#844, run 32394307803(수정전)·32396445235(수정후)·32397876960(배포)
...
### 2026-08-20 · A(260820) · BBE-252 전수재검증 — diff 0 아님, 기수별 GO/HOLD 분리
- 의도: belie 직접 지시(BBE-258 권장 후속, 신규카드 없이 BBE-252 이어감) — dashboard-parity.mjs --cohort "4,6,7,10" 전량 재실행(표본→전수 격상).
- 한 것: 기존 도구 그대로 실행(코드변경 0), run `32389628088`(headSha `2e8f8a2`, SSH 1회). **사용자 21명·diff0 10명·총diff 53건** — BBE-258(sales 원행)과 달리 이 정본 도구는 계약 집계(R1:U6 계약열·N.주N계약·H.주N활동·B21 누적수임비)까지 봐서 진짜 드리프트가 나옴. 기수별: 4기 1/1(100%) 클린·10기 5/6(83%)·7기 4/8(50%)·**6기 0/6(0%) 전원 불일치**. 6기는 B21 지문차집합이 구체 계약(구분=일반, 이월아님)을 특정 — 원인 미확정(DB중복적재 vs 시트편집후 미러누락, 둘 다 가능성 열어둠). 10기 유일 문제자는 최근 3일치 날짜라 async지연 의심(가설).
- 결정: **판정 = 기수별 분리.** 4기만 GO(스위치켜기 절차+되돌리기+확인3항목 BBE-252에 게시). 10기는 재실행으로 지연가설 반증 필요. 6·7기 HOLD.
- 다음: 6기 전용 후속카드 권장(계약 5~6건 표본 BBE-258식 심층해부). 10기는 며칠 후 재실행해 그 1명만 diff0 되는지 확인.
- SoR: BBE-252 Linear 코멘트, run 32389628088
...
| 트랙 | 역할·구역 | 현재 몸(버전) | 상태 |
|---|---|---|---|
| DC | **배차 허브(에이전트팀)** — 미배정·정체 이슈 발굴·배차·진행 감시·레인 충돌 판정·거짓완료 적발 + 필요 시 P0 직접 집행 | 경영일지 DC00(260827) | **시연 지연 P0 완주(2026-08-31)** — 근인 확정: 활성 129명 중 91명(71%) 레지스트리 캐시 컬럼 미완비, 범인은 아레나 기수(A2-* 전부 0/N). [🔄 동기화]로 134건 충전(완비 38→120) → **페이지 40초+→580ms**, 워밍 **61,521→489ms·allOk true**(#912 로 미완비자만 워밍). 관측수단 3종 신설(health warm 블록·VPS 로그 워크플로·cache-warm-diag). belie 지적 2건 수용·정정. 이하 이전: 미팅예약 달력 P0 완주·트래커 이관·#882 배차. 다음 = #913 아레나 자동충전 · #914 앱 로그 캡처. |
...
| E | 검증·지원 축(R7) — 승계(구 DevE 배포설정 잔여 = ADMIN_DRIVE_REFRESH_TOKEN 미등록 belie 대기, 별건 보류) | 경영일지 데탑 C작업원E(260820) | **BBE-242 로딩보고서 «캐시 절» 완주(2026-08-27)** — 읽기 전용 조사(코드 변경 0건). 서버 unstable_cache 5인스턴스(registry/cohorts 60초·share-scores/announcements 10분·**scoreboard 30분×35+명×3=최대105엔트리**) + 커스텀 SWR 1개(profile-bundle-cache, FRESH10분/GRACE30분, 개인 `/api/me`도 이 경로) + 클라이언트 React Query 8훅(staleTime 30초~1시간·gcTime 기본 5분) 전수 지도화. 가장 무거운 후보=전광판 30분 캐시(실측 미실시, SLO루프 7경로에 없었음 — A에 8번째 경로 추가측정 권장). 처방 4+1(SWR이식 권장1순위·캐시워밍·TTL지터·집계사전계산·클라 gcTime상향) + 부수발견(admin/users 문서드리프트, 옛 캐시명 "me-bundle-v2" 잔존). A에게 전달 + BBE-242 코멘트 게시. 이하 이전: **BBE-250 완주(2026-08-20)** — sheet_rows.payload GIN 인덱스 제거(미사용 확정). PR #823(`efa7cce`) 머지·배포 success·health 200, hosted 적용 run 32365152632. 이하 이전: **BBE-247 완주(2026-08-20)** — registry DB 읽기 60초 캐시 복원. PR #815(`68a8a54`) 머지·배포 success·health 200. 이하 이전: BBE-91 Done, BBE-42·BBE-77 완주 도장, BBE-67 부분, BBE-85 사후검증. 다음 배차 대기 |
...
### 2026-08-24 · 경영일지 데탑 C작업원D(260820) · BBE-245 시트독립 프로그램 종료 — 시트=백업전용 선언, 전 등록기수 DB 정본 전환 완료
- 의도: belie 직접 상시집행 배차 — "비파일럿 전환 스위치" 프로그램. 4→10→7→6기 순서로
  전환(각 단계 belie 명시 승인), 마지막 기수(6기) 전환 직후 확인 3항목 통과 즉시 종료
  선언까지 한 번에 이어가라는 지시.
- **한 것 — 4개 기수 전환(각 PR·배포·parity 재확인·BBE-75 도장 완주)**:
  - **4기**(PR [#846](https://github.com/bbelieff/salespt-log/pull/846), `eeecb5a`) — 전수 1명
    클린. 즉시 GO.
  - **10기**(PR [#859](https://github.com/bbelieff/salespt-log/pull/859), `996a58d`) — 5/6
    클린, 1명 미확정(08-17 현수막 미팅 상태값) belie 확인 대기로 문서화, 전환은 안 막음.
  - **7기**(PR [#861](https://github.com/bbelieff/salespt-log/pull/861), `6ae4770`) — B 가
    B21/주차집계 STATS_WEEKS(8) 클램프 근인 확정·수정(PR #848·#850) 후 5/8 클린, 잔차 3명은
    belie 승인 감수. **부수**: cohort "7"이 테스트 스위트 14개 파일에서 "비파일럿 예시"로
    하드코딩돼 있어 전환 후 check.sh 전체 실패 — cohort "1"로 마이그레이션.
  - **6기**(PR [#870](https://github.com/bbelieff/salespt-log/pull/870), `89d31cf`, **프로그램
    마지막 기수**) — B 가 소스정렬 공식(04 미팅.L 기준, PR #860·#862) 정합성 실증 후, 남은
    집계 잔차의 근인이 **시트 자신의 SUMIFS stale 캐시**(삭제된 과거 미팅 행의 결과를 재계산
    없이 계속 표시 — 1명은 원행 0건인데 캐시가 5,400,000 표시)임을 확정 → GO 판정. 전환 후
    실측 1/6 클린, 잔여 5명 전부 sheet>db 방향(유령값 제거)으로 예측과 정확히 일치.
- **⑤ 시트=백업 전용 선언**: `CLAUDE.md` §2.5 기술 제약 갱신 — "SSOT는 Google Sheets" 원칙을
  "SSOT는 Postgres DB, 시트는 비동기 미러·백업 export 전용"으로 대체. 요청 경로 시트 동기
  호출 재유입은 CI 가드(BBE-251, `sheets-request-path-guard.test.ts`)가 계속 차단. **전환
  스위치(`DB_READ_COHORTS`)는 삭제하지 않고 존치** — 문제 시 기수 라벨 1개만 Set 에서 빼면
  즉시 시트 정본 복귀(안전선, 운영에 비노출). 시트 파일·Sheets 연동 코드 자체도 보존.
- **범위 밖(정직 명기)**: 1·2·3·5기(registry 미등록 레거시) — 이번 프로그램 조사·전환 범위
  밖. 5기는 BBE-67 로 별도 census·백필 승인이 이미 진행 중(FOREMAN 큐), 1·2·3기는 미착수.
  "전 등록기수 전환 완료"는 정확한 표현이나 "전 기수"(무제한)는 아님 — §0.8 완료의 정의.
- **다음**: BBE-245 프로그램 카드 Done 전환, BBE-75 완주 도장(이어서 진행). 1·2·3기 legacy
  편입은 별도 향후 카드(belie 판단).
- SoR: BBE-245(Done 전환 예정)·BBE-252(전체 경과)·BBE-251(가드) · PR #846·#859·#861·#870
