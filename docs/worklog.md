...
| E | 검증·지원 축(R7) — 승계(구 DevE 배포설정 잔여 = ADMIN_DRIVE_REFRESH_TOKEN 미등록 belie 대기, 별건 보류) | 경영일지 데탑 C작업원E(260809) | **BBE-91 Done(2026-08-10)** — Linear 이번 세션 인증 확인, 직접 완주 코멘트+Done 전환. 재실행(반장 PR #752 렌더옵션 수정 반영 이후, run 31329451352→31329479835): `users 144·cohorts 13 ✅일치`(카드 기준 143과 +1, 8/8 이후 신규등록 추정·중복 2쌍 동일인 A1-1김덕호·A1-4박준용 불변). BBE-56 에 게이트 ON 입력 숫자 코멘트로 전달, 게이트 ON 자체는 A 트랙 소관이라 미착수. 이하 이전: **BBE-42·BBE-77 완주 도장 준비 완료(2026-08-09)** — BBE-77 재확인, BBE-42 "55/55 이관" 독립 재검증(repair-shifted-dryrun run 31326902052, 복구대상·잔재 0). 이하 이전: BBE-67 부분(cohort 1·2·3·5 매칭 0건) + BBE-85 사후검증(run 31303933202). 다음 배차 대기 |
...
### 2026-08-11 · 경영일지 데탑 C작업원B(260809) · BBE-67 5기 legacy 어댑터 구현 — PR #807 오픈, 머지 보류(dry-run 은 배포 후)
- 의도: belie 직접 지시 — "정밀 census 먼저, 그다음 어댑터. 590 dry-run 은 어댑터 완료
  후에만. G작업원D dry-run PASS 와 이 몸의 이전 STOP 보고 둘 다 유효, 다시 조사하지
  마라. 구조가 다르다는 건 이미 확정 — '같을 것'이라 가정하고 짜지 마라."
- 한 것: `scripts/ops/backfill-sheet-rows.mjs`에 읽기 전용 어댑터 2개 추가 —
  `parseLegacyDbManagement`(위치 고정, 매입DB/직접생산/현수막/지인·기고객·소개 → 모던
  03 DB관리 컬럼 문자 재매핑, 대응 없는 legacy 필드는 버림) · `parseLegacySalesRows`
  (순차 스캔, 소스마다 컬럼 세트가 달라 헤더 라벨로 매 주차 재탐색, 날짜는 매입DB 행
  에서만 읽어 carry-forward, `--legacy-year` 필수 인자). `isMainModule` 가드로 순수
  함수 import 시 CLI 부작용 격리. `backfill-sheet-rows.d.mts` 신설(테스트 typecheck
  통과용, .mjs 임포트 타입선언).
  **자체 발견·수정한 버그 2건**(구현 중 합성 테스트로 잡음, 실측 없이 넘어갔으면
  매입DB/현수막 섹션의 "주문금액" 값이 모던 "부가세여부" 컬럼에 잘못 새어들어갈
  뻔함) — `LEGACY_DB_SECTIONS`의 매입DB/현수막 `modernCols` 중 대응없음 슬롯이
  실제 컬럼 문자("F"/"U")로 잘못 채워져 있던 것을 `null`로 수정.
  **테스트 fixture 재작성**: 첫 시도의 헤더 배열이 요일/날짜/채널(0~2열) 오프셋을
  안 넣어 지표 컬럼 인덱스가 어긋났던 것도 자체 발견·수정(콜지기소 분류행은 "라벨만
  있으면 유효"가 의도된 동작임도 재확인 — 버그 아님, 기대치 쪽을 고침).
- 검증: `tests/ops/backfill-legacy-adapter.test.ts` 신규 14케이스(합성 fixture,
  PII 없음) green. 이 워크트리 `node_modules`가 사실상 비어있던 환경 문제 발견 →
  `npm ci` 로 해소. `check.sh` 전체 green(typecheck·lint·structural·unit 1330·
  doc-drift). PR **#807** 오픈, 본문에 census 표(4탭 상태·근거)·설계 근거·잔여
  불확실성(contracts 미검증) 명시. **머지는 하지 않았다** — 이 lease 규정("머지는
  검수 PASS 후 반장이 넣는다") 이행.
- 결정: 실 시트 dry-run(수용기준 ③)은 이 PR 밖 — `db-backfill.yml`이 git ref를
  checkout 안 하고 VPS 배포본을 그대로 실행하는 구조(BBE-69 와 동일 제약)라 머지+
  배포 후에만 가능. 머지되면 이 몸이 직접 `gh workflow run "DB Backfill" -f cohort=5
  -f legacy-year=<YEAR>` 돌려 결과(목표: source 8/8·sales 515·contracts 37·db 38·
  total 590·read error 0)를 BBE-67 에 보고하기로 함. FM §3(A) 샘플체크도 같은 dry-run
  으로 수행.
- 다음: 반장 머지 대기. 머지 후 이 몸이 dry-run 트리거+결과 보고, contracts 탭 행수
  (37) 검증까지 이어감.
- SoR: PR #807 · Linear BBE-67 코멘트(census + 이번 완료 보고) · `scripts/ops/
  backfill-sheet-rows.mjs`.
