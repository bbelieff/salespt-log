...
| 트랙 | 역할·구역 | 현재 몸(버전) | 상태 |
|---|---|---|---|
| DC | **배차 허브(에이전트팀)** — 미배정·정체 이슈 발굴·배차·진행 감시·레인 충돌 판정·거짓완료 적발. 코드 미작성(소유 구역 없음), 산출물은 worklog·이슈 코멘트뿐 | 경영일지 DC00(260827) | **#882 배차 게시 + 온보딩 템플릿 수리(2026-08-27)** — 트래커 이관 완료(미완료 13장 전량 GitHub, Linear 끊음) 후 **#882 배차 프롬프트를 카드 코멘트로 게시**(크래시 먼저→Suspense→프리페치 순, 레인 충돌 확인 완료: #881·#888 과 안 겹침, 병렬 가능). **Hashimoto 수리**: `docs/playbooks/worker-onboarding.md` 가 끊긴 Linear(BBE-75·73)를 보라고 지시하고 있어 치환 17건으로 GitHub 기준 갱신(하트비트→#889, 도장→보드 README+완주증거 필드) — 안 고치면 모든 신규 세션이 같은 데서 헛돈다. **부수 발견·수리**: 이 템플릿과 `thinking-protocol.md` 가 **origin 에 없는 로컬 전용**이었다(23KB) — 온보딩 정본이 git 밖이라 리셋 한 번에 소멸할 상태여서 레포로 편입(PR #891). 배차 대기열 = **#882**(최우선) → #887 → #881 → #888 → #884. |
