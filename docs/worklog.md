...
### 2026-08-09 · 데탑 C작업원A(260809) · BBE-56 반장 머지 확인 + 후속 PR 2건 — 게이트는 여전히 OFF
- 실측: PR #743 이 반장에 의해 머지됨(`469ed87`) — 배포 run
  [31315301133](https://github.com/bbelieff/salespt-log/actions/runs/31315301133). 게이트
  `REGISTRY_DB_READ` 는 deploy.yml 주입 목록에 없어 **이 배포로 동작 변화 0**(PR 본문 명시).
- ⚠️ 배포 판정 정정: 이 run 의 "Public health check" 스텝이 실패(GH 러너 curl 이 134초 뒤
  접속 실패, rc=28)했으나 같은 run 의 "Deploy on VPS" 내부 VPS 로컬 `/api/health`=200 은
  정상(무중단 컷오버 완료), 직접 재확인도 200 — **실제 장애 아님**. 오늘 2번째(3d7f463 도
  동일 패턴) — GH러너→공개도메인 순간 접속 실패로 보고 별도 태스크로 분리(deploy.yml
  Public health check 에 재시도 로직 부재, BBE-77 계열 가능성 열어둠).
- 반장 머지 시점 브랜치에 없던 커밋 2건을 후속 PR 로 분리(리베이스가 "이미 적용됨"으로
  인식해 자동 스킵한 것 확인 후 재오픈):
  - **#752** — BBE-56 코멘트에서 반장이 지적한 `backfill-registry.mjs` 의
    `dateTimeRenderOption` 누락(→ 날짜가 앱과 다른 형식 `SERIAL_NUMBER` 로 적재) 수정 +
    구조 테스트(반증 확인: 되돌리면 3건 중 2건 실패).
  - **#756** — 게이트 ON 판단용 대조표 도구 `registry-parity.mjs`(belie 지시 "시트 시절과
    같은 값인지 대조표로 남겨라"). `dashboard-parity.mjs` 패턴 계승, `db-audit.yml` 화이트
    리스트 추가. 백필 전이라 아직 미실행.
  - 둘 다 CI green, check.sh 초록. 머지는 반장 판정 대기.
- 다음: #752·#756 머지 → BBE-91 백필 dry-run→execute(시트 고유키 users143·cohorts13 기준
  대조) → `registry-parity` 실행 → 불일치 0건 확인 후 게이트 ON → `/admin/users`·수강생
  상세·클레임 흐름 스모크. 다르면 즉시 OFF.
- SoR: PR #743(머지)·#752·#756 · Linear BBE-56·BBE-91
