...
| B | **작업원 B (belie 명명 2026-08-05)** — 반장 배차 수행. 구 gcal 계열 잔여 승계 | DevB(260805) | **승계(260703→260805) · 배차 대기** — 이 몸은 A(260803) 창이 belie 지시로 B 로 개명·전환된 것. 인계 완료분(A 명의 §6.8 완주 4건): #652·#653·#655·#686 전부 배포 success·health 200. 구 B 잔여 = gcal pm2 침묵 인프라(VPS 접근 필요) — 그대로 승계. **점유 레인 없음 · 즉시 배차 가능.** 반장 앞 보고 3건 = 아래 2026-08-05 DevB 로그 항목 |
...
3-b. [ ] 🔺**A2 배치 — 실행 권한(VPS)은 열렸으나 토큰 권한범위 문제로 재차단** (FM · due 8/6, D-1).
   VPS 원격 실행(#679~682)까지는 뚫렸고 `--season-row`·`--plan` 은 정상 성공. 그런데 실제 시트
   복사(`--canary`)가 **서로 다른 사람 3명(손기학·김지훈×2)에서 전부** `PERMISSION_DENIED(forbidden)`.
   **읽기(files.get·about.get)는 전부 정상, 쓰기(files.copy)만 전부 거부** — 특정 파일 문제가 아니라
   **토큰 권한범위(scope) 문제로 확정**. 정황: ADR-0015 발급 스크립트(`scripts/get-admin-drive-token.mjs`)
   는 `scope: drive`(전체)를 요청하도록 코드에 박혀 있는데, 실제 재발급 시 **belie 화면의 동의 체크박스가
   일부만 승인됐거나(구글이 "drive" 를 세부 항목으로 쪼개 보여줌)**, 이 스크립트가 아닌 다른 방식으로
   재발급됐을 가능성.
   · **belie 액션(5분)**: 이 PC 나 belie PC 에서 `node scripts/get-admin-drive-token.mjs` 재실행 →
     뜨는 구글 동의 화면에서 **"Google Drive의 모든 파일 보기·수정·생성·삭제" 항목이 체크돼 있는지
     직접 확인** 후 동의 → 클립보드에 복사된 값을 GitHub Secret `ADMIN_DRIVE_REFRESH_TOKEN` 에 붙여넣기.
   · 되돌리기: 불필요(기존 값 그대로 두고 실패만 계속되는 것과 같은 안전 상태 — 토큰 교체는 위험 없음).
   · 미루면: 8/7 개막까지 시트 55개 미생성 — **차선책**: 개강일 텍스트만 공지하고 시트 복사는 준비되는
     대로 별도 전환(전광판 개강일 칸만 나중에 채워도 안전 — §7-6② 로직).
...
- ADR-0015 확인: 정본 발급 경로 `scripts/get-admin-drive-token.mjs` 는 `scope: drive`(전체 읽기·쓰기)를
  ...
  세부 체크박스가 일부만 승인됐을 가능성으로 좁혔다(코드로는 더 좁힐 수 없음 — 토큰 자체를 안 보고는
  실제 부여 scope 를 조회할 API 가 없다).
- 결정함 3-b 갱신: belie 액션 = 해당 스크립트로 재발급 + 동의 화면에서 전체 항목 체크 확인.
  차선책(개강일만 공지, 시트 복사는 준비되는 대로)도 함께 기재 — 8/7 D-2 이므로 무기한 대기는 안전하지 않음.
