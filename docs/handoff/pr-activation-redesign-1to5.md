...
> **⚠️ 갱신(2026-06-21 감사 후)**
> - **PR1(생산 E를 DB생산 집계로 이전)은 이미 구현됨**(ADR-0020). 컨택은 F:H만 쓰고 DB집계가 E를 소유. 다시 만들지 말 것.
> - 채널별(매입DB·직접생산·현수막·콜지기소) DB생산↔컨택 연동의 **검증·해결 완료 상세 + 전체 개발 프롬프트는 [pr-db-channels-full.md](./pr-db-channels-full.md)** 로 이관. 아래 PR2~PR5 요약은 그 문서가 정본.
