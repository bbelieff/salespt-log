...
> **📌 후속 갱신 (2026-08-05 17:15 UTC)** — belie가 GCP 콘솔을 직접 확인한 결과 **①도 원인이
> 아니었다**(이미 프로덕션 게시 상태). 실제 근인은 **③의 다른 층위** — 아래에서 배제한 "원본
> 파일의 `copyRequiresWriterPermission`"이 아니라 **"복사본이 저장될 목적지 폴더"의 소유자·공유
> 방식**이었다(`copySheet()`가 `parents`를 지정하지 않아 원본과 같은 폴더에 사본을 만들려 했는데,
> 그 폴더가 admin OAuth 계정과 다른 표기 계정 소유 + 링크공유뿐이라 API 쓰기가 거부됨). belie가
> 해당 폴더에 관리자 계정을 편집자로 추가해 조치 완료, FM 재시도 중. 상세 절차는
> [`docs/playbooks/oauth-console-check.md`](../playbooks/oauth-console-check.md) §③에 등재.
> 아래 ②③ 실측(파일 자체의 저장용량·개별 정책)은 여전히 유효한 배제이며, 이 갱신은 "폴더 계층의
> 권한"이라는 **추가로 봐야 했던 층위**를 기록해두는 것이다.
