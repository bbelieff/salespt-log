...
- 모바일 Option A(Quick Action Dock)는 상단의 선택 범위 라벨과 압축된 `DB 비용 / 추가 비용 / 총비용` 요약, `기록 / 조회 / 관리` 작업 탭, 스크롤 영역, 기록 화면의 하단 고정 저장 CTA로 구성한다. 팝업 요약은 월·전체·카테고리 조회 응답의 `selectedScope`와 totals를 사용하며, 외부 대시보드 비용 카드는 기존 과정 누적 범위를 유지한다. 360px·390px에서도 각 터치 표적은 최소 44px을 유지한다.
...
### ExpenseLedgerTable (2026-07-24)

- `ExpenseLedgerDialog` 내부의 선택 범위 서버 집계 표다. 월별·전체 조회는 사용자 비용과 DB 시스템 행(`매입DB / 직접생산 / 현수막`)을 함께 렌더링한다. 시스템 행은 `자동 집계 · 읽기 전용`으로 표시하고 disclosure 안에도 mutation 컨트롤을 두지 않는다.
- 인식 방식은 서버의 `allocated / recognized_on_date / recognized_on_start / unallocated`를 그대로 설명한다. 미배분 행은 빈 날짜를 날짜처럼 계산하지 않고 `인식일 미지정`으로 표시한다.
- 목록 아래 footer는 선택 범위의 `DB 비용 / 추가 비용 / 총비용` 세 줄만 표시하며 상단 카드와 같은 서버 totals를 사용한다. `totalCost === 0`이고 entries도 0건일 때만 empty를 표시한다.
- 카테고리 조회는 서버 `categoryTotals`의 시스템·사용자·미분류·보관 행을 비용 내림차순으로 보여 주고 `카테고리명 / 비중 / 항목 수 / 총비용`을 표시한다. 시스템 카테고리는 기록 picker나 이름 변경·보관 surface에 합치지 않는다.
- 로딩, 성공 empty, 로그인·권한·일시 오류를 서로 다른 상태로 표시한다. 오류 원문을 노출하거나 0원/빈 원장으로 대체하지 않고 안전한 한국어 안내와 명시적 재시도를 제공한다.
