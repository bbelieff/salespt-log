...
| **TraineeCard** | /admin/users 의 개별 수강생 카드 (CohortSection + 팀 박스 안에서 사용). Props: `u` (Trainee), `archived`, `viewOnly`, `busy`, `nameByEmail`, `linkedBySheet?`, `onPick`, `onReserve`, `onSetTeam`. 인라인 팀명 input (Enter/blur 자동 저장) + [유보][📊 시트 ↗][웹앱 →]. POST /api/admin/set-trainee-team 트리거. 500줄 cap 회피로 별도 파일. |
...
| **SortableTraineeBox** | 박스(같은 cohort+team) 내 TraineeCard 들을 dnd-kit 로 묶는 wrapper. PR C-1 (2026-05-13). Props: `members`, `onReorder?(emails)`, 그 외 BoxCommonProps. DndContext + SortableContext (verticalListSortingStrategy) + 3 Sensors (Pointer distance 5, Touch delay 200ms, Keyboard). onDragEnd 시 local items state 즉시 갱신 (optimistic) + onReorder(new order emails) 호출. onReorder 미제공 / viewOnly / archived → plain TraineeCard 렌더링 (dnd 비활성). members prop 갱신 시 useEffect 가 local state 재초기화 (router.refresh 동기화). |
