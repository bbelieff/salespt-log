...
- 모달은 DB 비용 합계(읽기 전용), 추가 비용, 총비용을 분리 표기한다. 추가 비용 상태를 읽지 못하면 총액을 0으로 대체하지 않고 `확인 필요`로 표시한다.
- 입력은 카테고리 선택·즉시 추가, 품목, 부가세 제외 원화 정수, 당일/포함 기간, 매월 반복을 제공한다. 기간은 일할 인식 미리보기를 함께 보인다.
- 조회는 월별(이전/다음 월 버튼), 전체, 카테고리이며 표와 서버 계산 합계를 표시한다. 카테고리는 이름 수정, 보관, 복원을 지원한다.

### ExpenseLedgerTable (2026-07-23)

- `ExpenseLedgerDialog` 내부의 서버 집계 표다. 월별·전체·카테고리 조회에서 항목의 카테고리·품목·인식 기간·서버 계산 금액을 보이고, 하단은 카테고리별 합계와 추가 비용 합계를 다시 표시한다.
