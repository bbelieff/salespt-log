...
| **CompanyInfoEditor** | 미팅 업체정보(04 T~AN) 드롭다운+팝업 편집. Props: `value?: CompanyInfo`, `onSave(ci)`, `busy?`. [업체]12+[대표자]8+커스텀(필드추가+). 사용처: schedule MeetingResultCard·contact SavedItem(예약비고 자리 교체, patch({업체정보}))·payment(CompanyInfoContractSection 경유). consultation-log §3. |
