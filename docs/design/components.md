...
- **WeeklyGoalSummary**: dashboard/DB생산/contact/schedule의 같은 주간 집계 API를 사용하는 링 요약. 컨택과 일정은 선택 날짜의 금~목 주간을 따른다.
- **GoalRings**: 생산·유입·컨택완료·미팅완료·계약 실적/목표. null은 미기재, 0은 목표 0, 양수만 달성률. 기존 brand-red/green/gray 토큰.
- **WeeklyGoalPage**, **WeeklyGoalEditor**: 학생/담당 트레이너 공통 화면, PC 지난주 비교와 현재 편집 나란히, 모바일390 입력. DirtyGuard와 revision 충돌 보호.
...
| **CohortPendingRetryButton** | Admin 전용 "기수 생성 대기 재시도" (`/admin/cohorts` 헤더, R3-5 phase-2b). Props: `pendingCount: number`. Drive 복제 실패로 pending 큐에 쌓인 멤버를 `POST /api/admin/retry-cohort-creates` 로 완주(복제→prep→roster→O1/O2, 멱등). pendingCount 0 이면 숨김. 결과=완료/대기/남음 표시. |
| **CohortCreateModal** | Admin 전용 "기수/참가자 추가" 모달 (`/admin/cohorts` 헤더). Props 없음. 기수 토큰(8/a1) + 모드(생성=템플릿복제 / 연동=기존시트) 토글 + (생성) 템플릿/루트폴더/(아레나)명단 시트 ID + **수강시작일(선택, date)** + 참가자 줄단위 입력 → POST /api/admin/create-cohort-members → 생성/건너뜀/**대기(pending)**/실패 리포트. 수강시작일 넣으면 새 시트 O1/O2 자동 세팅(R3-5). parseCohortToken 으로 클라이언트 토큰 미리보기. **날짜 안전장치(사고 2회 후)**: ① 제출 시 `resolveCourseStartInput` 로 state→input DOM value **ref 폴백**(자동화가 값을 직접 넣어 onChange 가 안 떠도 전송) ② create+날짜 빈값이면 **앰버 확인 1회** 후 재클릭해야 진행 ③ 응답 `dates[]` 를 초록(기록: O1→O2)/앰버(미기록 + 시트 실측 O1·O2·B3) 블록으로 표시. |
...
### ExpenseCategoryPicker (2026-07-24)

- 비용 기록 위치의 접근 가능한 combobox/popover다. 선택 버튼은 `role="combobox"`, `aria-expanded`, `aria-controls`를 제공하고 열린 목록은 검색 가능한 `listbox/option` 구조를 사용한다.
- 같은 popover 안에서 새 카테고리 추가와 선택 카테고리 이름 변경·보관을 수행한다. 사용 이력을 보존해야 하므로 삭제는 실행하지 않고 보관 안내를 반환한다.
- 활성 카테고리만 선택 후보로 보이며 검색·선택·관리 컨트롤은 모바일에서 최소 44px 터치 높이를 유지한다.
- 카테고리 query의 초기 로딩, 성공한 빈 결과, 로그인 만료·권한 부족·일시적 실패를 서로 다른 상태로 표시한다. 실패 원문은 노출하지 않고 안전한 한국어 안내와 `다시 시도`를 제공하며, 실패를 빈 카테고리 목록으로 대체하지 않는다.
...
- 조회는 월별·전체·카테고리 전체 비교를 제공한다. 관리는 반복 규칙과 카테고리를 목록 행의 `상세·관리` disclosure로 펼쳐 작업한다. 임시 생성 직후 전용 알림 박스는 두지 않는다.
- 관리의 카테고리·반복 규칙 query는 로딩·성공 empty·권한/오류를 구분한다. `401/403/503` 및 기타 실패는 0건이나 “등록된 항목이 없음”으로 표시하지 않고 안전한 복구 안내와 명시적 재시도를 제공한다.
- active·paused 반복 규칙의 `삭제/종료`는 물리 삭제가 아니라 서버의 terminal soft archive다. 행 안에서 앞으로의 발생 중단과 과거 비용 기록 보존을 명시하고 재확인한 뒤 실행한다. 실패 시 행과 확인 상태를 유지한 채 안전한 권한·로그인·일시 오류 안내와 재시도를 제공하며, archived 규칙에는 삭제·중지·재개·건너뛰기·미래 금액 변경을 노출하지 않는다.

### ExpenseLedgerTable (2026-07-23)

- `ExpenseLedgerDialog` 내부의 서버 집계 표다. 월별·전체 조회는 모바일 목록 행을 펼쳐 항목의 기간·발생 방식과 관리 이동을 보여 주고, 하단은 서버 계산 카테고리 합계와 추가 비용 합계를 표시한다.
- 카테고리 조회는 선택 필터가 아니다. 현재 전체 범위의 모든 카테고리(보관·미분류 포함)를 총비용 내림차순으로 정렬하고 `카테고리명 / 비중 / 항목 수 / 총비용`을 표시한다. 비용이 없는 카테고리도 0원·0건으로 남긴다.
- 로딩, 오류, 빈 결과를 서로 다른 상태로 표시한다. 오류를 0원 또는 빈 원장으로 대체하지 않는다.
