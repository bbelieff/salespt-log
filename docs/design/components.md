...
| **UpdateAccordion** | 업데이트 묶음 아코디언 (§7-4, grouped-updates 개편). Props: `updates`(행) — groupUpdates 로 항목(그룹=1)화. 접힘=고객용 제목+메타줄(그룹 "개발 M/D~M/D · 적용 M/D"/단건 "적용 M/D"), 펼침="339·340…번째 업데이트" + [전]/[후] 뱃지 문단(`전:`/`후:` 포맷, 없으면 MarkdownView 폴백). 팝업·보관함 공용. |
