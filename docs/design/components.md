...
- 로고·역할 버튼은 `h-11`(모바일 44px). 역할 전환 바깥 박스는 상하 2px 여백 포함 48px이며, 대시보드 링크도 `h-12`(48px)로 외곽 상하를 맞춘다. 대시보드에는 LayoutDashboard 아이콘과 라벨을 함께 표시한다. PC에서는 전역 루트 스케일이 적용된다.
