...
| **GcalConnectCard** | 구글 캘린더 연동 카드 (gcal-1, google-calendar-sync §4-1 문구 정본). Props 없음 — /api/gcal(GET 상태·POST 설정·DELETE 해제) fetch. 미연결/연결됨/실패("연결이 풀렸어요") 3상태. 연결됨: 캘린더 드롭다운 + 토글 3종(미팅/실무/일반). [연결하기]→/api/gcal/auth. 금지 용어(동기화·토큰·OAuth) 화면 미노출(ADR-0028). |
