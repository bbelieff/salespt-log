...
| **CompanyInfoEditor** | 미팅 업체정보(04 T~AN) 드롭다운+팝업 편집. Props: `value?: CompanyInfo`, `onSave(ci)`, `busy?`. [업체]12+[대표자]8+커스텀(필드추가+). 저장 시 호출부가 `patch({업체정보})`. contact/schedule/payment 공용. consultation-log §3. |
