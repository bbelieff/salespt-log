...
| **TrainerMgmtSections** | TrainerMgmtPanel 의 SectionPending / SectionAssign / SectionManagement + parseAssigned, groupByCohort, PanelUser 유틸. 명단 섹션은 TrainerMgmtListSections 에서 re-export. |
| **TrainerMgmtListSections** | 명단 표시용 — SectionTraineeList (수강생, 담당 트레이너 이름 매핑) + SectionTrainerList (트레이너 카드, 담당 수강생). 파일 크기 가드. |
