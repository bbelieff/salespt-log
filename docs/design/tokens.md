...
| **일반이벤트 teal** ⭐ | **#0d9488** (teal-600) | 캘린더 **일반이벤트**(type=일반, 기존고객/개인 일정 — 비집계) pill 배경 · 일자상세 좌측바 · 레전드. 채널 4색·실무 진회색과 구분되는 신규 hue (consultation-log §4-2). inline style. |

**type 아이콘 5종** (lucide 모티브 인라인 SVG, `TodoTypeIcon`): 미팅=사람들(users) · 전화=수화기(phone) · 메시지=말풍선 · 기타=점3(•••) · **일반=캘린더**. 배경색(진회색/teal) + 흰 아이콘.
...
**2026-07-15 — 카드 = 다크 → 라이트 프로스트 글래스로 전환.** 링 안 마크가 `S` 글자 →
**세일즈PT 로고 원본**(`/salespt-logo.png`, 무가공)으로 바뀌었고, 로고의 "세일즈"·영문 글자가
검정이라 다크 카드에서 묻혔다. **에셋을 고치지 않고 배경을 바꾸는 것이 정본**(belie 지시).

| 용도 | 값 | 비고 |
|---|---|---|
| z-index | `z-[400]` | 모달(z-50대)보다 위 — 로딩은 최상단 |
| dim 배경 | `rgba(10,10,14,0.55)` + `blur(2px)` | 전체 덮기(safe-area 무시). **어둡게 유지** — 라이트 카드 대비 확보 |
| 글래스 카드 | `rgba(255,255,255,0.82)` + `blur(12px)` | **반투명 라이트**(구: `rgba(22,22,28,0.72)` 다크) |
| 카드 테두리·글로우 | `0 0 0 1px rgba(0,0,0,0.06)` + `0 8px 40px rgba(215,22,23,0.18)` | 흰 카드용 다크 헤어라인 + **톤다운** 빨강 글로우(구 0.28) |
| 문구 색 | `text-gray-800` | 다크 잉크 (구: `text-white/90` — 흰 카드에선 안 보임) |
| 링 지름 | `h-24 w-24` = **6rem** (모바일 96px · **PC 81px** — 아래 PC 밀도 스케일 적용) | 로고(3:1 워드마크) 수납 위해 5rem → 6rem 확대 |
| 궤도점 회전 | **`.lo-orbit`(= 링과 같은 박스 `inset-0`) 자체를 회전** — origin 기본값(중심) | ⚠️점(`::before`)에 **px 고정 `transform-origin` 금지** — rem 링과 어긋나 PC 에서 점이 링 밖으로 튄다(2026-07-15 수리) |
| 링 안 로고 | `/salespt-logo.png` · `width:100%` + `padding: 0 0.75rem` · `object-contain` | **원본 무가공**. 실효 크기 ≈ 72×24px(모바일) — 헤더 로고(`h-6`=24px)와 같은 급. 장식(`alt=""`) — 문구는 오버레이 `aria-label` 이 읽음. 여백을 더 줄이면 호흡(scale) 때 링에 닿는다 |
| 링/점/라인 | `#d71617` (conic·점·sweep) | brand-red. 흰 배경에서 대비 상승 — 값 그대로 유지 |

- 모션: `prefers-reduced-motion: reduce` 면 키프레임 비활성 + 정적 링/문구(globals.css 폴백).
  `.lo-core` 호흡(`lo-breathe`)은 로고에 그대로 적용된다.
...
## 반응형 브레이크포인트 (2026-06-02 데스크탑 개선)

`tailwind.config.ts > theme.screens` — 모바일 우선(xs~2xl) + 데스크탑(pc/wide):

| 이름 | px | 기기/용도 |
|---|---|---|
| `xs` | 360 | Galaxy S 기본(최소 폭) |
| `sm` | 390 | iPhone 12~17e |
| `md` | 402 | iPhone 17/16 Pro |
| `lg` | 430 | iPhone Pro Max/Plus |
| `xl` | 480 | Galaxy Ultra |
| `2xl` | 768 | 태블릿 |
| **`pc`** ⭐ | **1024** | **데스크탑(노트북)** |
| **`wide`** ⭐ | **1280** | **와이드 데스크탑** |
