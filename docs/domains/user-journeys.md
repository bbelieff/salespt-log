...
- **목표**: 미팅 일정을 앱과 Google Calendar에 동시 등록하고, 미팅 후 계약 결과 기록
- **성공 기준**: Google Sheets '앱_미팅예약' 탭에 미팅이 등록되고, Google Calendar에 이벤트가 생성됨
...
flowchart TD
    ...
    M --> N[Google Calendar 이벤트 생성]
    N --> O[성공 토스트: 미팅 등록 완료 📅]
    
    P[미팅 당일] --> Q[일정·계약 탭에서 미팅 확인]
    Q --> R[미팅 완료 후 상태 업데이트]
    R --> S[계약여부 true/false 선택]
    S --> T{계약 성사?}
    T -->|YES| U[수임비 금액 입력]
    T -->|NO| V[미팅 완료 처리]
    U --> W[계약비고 입력]
...
**엣지 케이스**:
1. **시트 저장 성공 + 캘린더 실패**: 시트는 유지하고 "캘린더 동기화 실패. 일정은 기록됨 ⚠️" 경고 토스트
2. **중복 미팅**: 같은 날짜+시간+업체명 → MVP에서는 허용하지만 향후 경고 예정
3. **수임비 누락**: 계약=true인데 수임비=0 → "계약 시 수임비를 입력해주세요" 검증 에러
...
- Calendar API는 부가 서비스, 실패해도 핵심 기능은 동작