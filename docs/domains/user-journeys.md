> **📄 이 문서는 무엇인가요?**
> - **한 줄 요약**: 세일즈PT 영업일지 앱의 7개 핵심 사용자 시나리오와 각 단계별 상호작용 플로우 (유예 기간 포함)
...
## 시나리오 7: 8주차 이후 마감 유예 미팅 기록

- **페르소나**: 이수강 (PRM 5기 수강생)
- **트리거**: 수료 직후 8주차 마지막에 잡았던 미팅이 실제로 진행되는 날이 도래
- **목표**: 해당 미팅의 완료/계약/수임비를 마감 유예 기간 내에 기록
- **성공 기준**: 유예 기간 미팅이 정상 저장되고 별도 집계 영역에 표시됨

**단계 (Mermaid flowchart)**:
```mermaid
flowchart TD
    A[앱 실행 - 수료 후 접근] --> B[📌 마감 유예 배너 표시]
    B --> C[일정·계약 탭 선택]
    C --> D[캘린더에서 유예 기간 미팅 확인]
    D --> E[앰버색 배지: '📌 유예 기간' 표시]
    E --> F[미팅 상태를 '완료'로 변경]
    F --> G{계약 성사?}
    G -->|YES| H[수임비 입력 가능]
    ...
    H --> J[유예 기간 계약 저장]
    I --> K[유예 기간 미팅 완료]
    J --> L[수납관리 탭에서 수납 기록 가능]
    K --> L
    L --> M[별도 집계 영역에 '📌 유예' 표시]
    M --> N[8주 기본 통계와 구분하여 표시]
```

**엣지 케이스**:
1. **편집 기간 초과**: 10주 이후 접근 → "수강 완료, 기록 종료" 완전 읽기 전용
2. **유예 기간 통계**: 9~10주 데이터는 8주 통계와 별도 집계
3. **시각적 구분**: 유예 기간 데이터는 앰버색 배지로 구분 표시

---

## 향후 확장 기능

**⚠️ 이 섹션의 모든 기능은 MVP에서 제외되며, `docs/future/extensions.md`에서 자세히 다룹니다.**

### Google Calendar 동기화 (P2)
