...
- **N1**: 수강시작일 (1주차 시작, 금)
- **N2**: 수료일 (시트 디스커버리 2026-05-08 확정 — 수식 `=N1+57`, 종강총회와 같은 날)
...
| 5 | **미팅완료** *(정의 변경 2026-05-08)* | **결과가 정해진 미팅** (계약+완료+취소) | `04 업체관리!J` 상태 IN ("계약","완료","취소") COUNTIFS |
| 6 | 계약 | 계약 체결 | `04 업체관리!K = TRUE` 또는 `J = "계약"` COUNTIFS |

**단계별 정의** (사용자 결정 2026-05-08):
- **미팅예약** = 예약 + 계약 + 완료 + 변경 + 취소 (= 모든 미팅 약속, 04 업체관리 row 생성 시점)
- **미팅완료** = **계약 + 완료 + 취소** (= 미팅 결과가 정해진 케이스. 변경만 제외 — 변경은 새 row 생성)
  - ⚠ 이전 정의 "완료+계약 (실제 미팅 진행)" 은 폐기. 사용자 spec 따라 취소 포함.

**전체 전환율 인사이트**:
- **기준**: 유입 → 계약 (생산 제외)
- 공식: `계약 ÷ 유입`

## 생산성 지표 4개 ⭐ (대시보드 SSOT — 사용자 결정 2026-05-08)

| 지표 | 공식 | 의미 |
|---|---|---|
| **DB 퀄리티** | `컨택진행 ÷ 유입` | 들어온 DB가 부재/거절 없이 컨택된 비율 (DB 질) |
| **컨택성공률** *(구 "컨택숙련도")* | `미팅예약 ÷ 컨택진행` | 컨택 → 미팅예약 전환 |
| **미팅숙련도** | `계약 ÷ 미팅완료` | 미팅완료(계약+완료+취소) 중 계약 비율 |
| **영업생산성** | `계약 ÷ 컨택진행` | **종합 지표** — 통화부터 계약까지 |

**시트 H 컬럼 통일 가이드** (별도 작업 — 사용자 시트 수정):
- 대시보드(자동작성)!H25:H29 (현재 5지표) → 4지표로 정리
- 라벨 + 수식 user spec 통일
- 시트 H 정정 가이드: docs/handoff/sheet-update/productivity-2026-05-08.md (별도 PR)
