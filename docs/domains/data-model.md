...
- 저장은 service의 실제 session admin 검증 → 자격 행 SHARE lock → active/exact key 확인 → 기존 version 비교 → SQL CAS → 같은 트랜잭션 감사 snapshot. 병행 수정은 409, 잘못된 입력 400, 자격·권한 없음 403, DB/auth 의존 실패 503. grade 변경은 서버에서 기본값으로 reset하고 동일 grade의 다음 저장부터 개별 grants를 유지한다.
...
| H | **미팅 예약건 수** ⭐ (구 "컨택성공 수") | **컨택관리** | 채널별 (직접 입력). 시트 디스커버리 2026-05-08: 헤더가 "미팅 예약건 수"로 변경됨. UI/코드 짧은 라벨은 `미팅예약` 별칭 사용 가능 |
...
- **E~F열 1-6행**: 생산/유입/컨택/미팅예약/미팅완료/계약 총합 (`=SUM(...)`)
...
> **변경**: 5단계 → 6단계로 재구성 (claude.ai prototype 핸드오프 2026-05-07).
> **본격 영업 퍼널 시작점**: 유입부터. 생산은 채널 운영 효율 영역이라 영업 효율과 분리.

| # | 단계 | 의미 | 데이터 출처 |
|---|---|---|---|
| 1 | 생산 | 채널별 DB 생산활동량 | `01 영업관리!E` 채널별 합계 |
| 2 | **유입** *(신규)* | 유효 DB 등록 | `01 영업관리!F` 채널별 합계 |
| 3 | 컨택진행 | 통화 시도 수 | `01 영업관리!G` 채널별 합계 |
| 4 | **미팅예약** *(구 컨택성공 / 시트 헤더 "미팅 예약건 수")* | 모든 미팅 약속 (상태 무관) | `01 영업관리!H` 채널별 합계 |
| 5 | **미팅완료** | 실제 진행된 미팅 (계약+완료) | **`01 영업관리!L`** 또는 `04 업체관리!J` 상태 IN ("계약","완료") COUNTIFS |
| 6 | 계약 | 계약 체결 | `04 업체관리!K = TRUE` 또는 `J = "계약"` COUNTIFS |

**단계별 정의** (사용자 결정 2026-05-08):
- **미팅예약** = 예약 + 계약 + 완료 + 변경 + 취소 (= 모든 미팅 약속, 04 업체관리 row 생성 시점)
- **미팅완료** = **계약 + 완료** (= 실제 진행된 미팅, 취소·변경 제외)
  - **시트 출처**: `01 영업관리!L` (시트 자동 수식 — 상태 IN ["계약","완료"]). matrix.미팅완료 = L값 그대로.

**전체 전환율 인사이트**:
- **기준**: 유입 → 계약 (생산 제외)
- 공식: `계약 ÷ 유입`

## 생산성 지표 5개 ⭐ (대시보드 SSOT — 사용자 결정 2026-05-08)

| # | 지표 | 공식 | 의미 |
|---|---|---|---|
| 1 | **DB 퀄리티** | `컨택진행 ÷ 유입` | 들어온 DB가 부재/거절 없이 컨택된 비율 (DB 질) |
| 2 | **컨택성공률** | `미팅예약 ÷ 컨택진행` | 컨택 → 미팅예약 전환 |
| 3 | **미팅실행률** | `미팅완료 ÷ 미팅예약` | 예약된 미팅이 실제 진행된 비율 (취소·변경 회피율) |
| 4 | **미팅숙련도** | `계약 ÷ 미팅완료` | 진행된 미팅 중 계약 비율 (미팅완료 = 영업관리!L) |
| 5 | **영업생산성** | `계약 ÷ 컨택진행` | **종합 지표** — 통화부터 계약까지 |

**시트 H 컬럼 통일 가이드** (별도 작업):
- 대시보드(자동작성)!H25:H29 라벨/수식 5지표 user spec 정정
- 가이드: `docs/handoff/sheet-update/productivity-2026-05-08.md`
...
// 코드(lib/types/index.ts) 식별자: ChannelDailyRow
// 4지표 코드 식별자: MetricKey (production/inflow/contactProgress/contactSuccess)
//   ⚠ 코드의 contactSuccess 명칭은 시트 헤더 "미팅 예약건 수" (구 "컨택성공") 와 매핑.
//      코드 follow-up 별도 PR(refactor/metric-key-rename)에서 contactSuccess → meetingReservation 으로 정리 권장.
type ChannelRow = {
  channel: ChannelKey;
  생산: number;        // 01 영업관리!E
  유입: number;        // 01 영업관리!F
  컨택진행: number;    // 01 영업관리!G
  미팅예약: number;    // 01 영업관리!H (시트 헤더 "미팅 예약건 수")
};
...
| 채널별 비용 (3채널) | `03 DB관리` raw row 서버 합산 | `lib/service/dashboard.ts` 가 `readPurchases.주문금액` / `readProductions.기간예산` / `readBanners.주문금액` 합산. 옛 `F56`/`K56`/`U56` SUM 셀 의존 제거 (2026-05-15 사고). |
...
interface DashboardCostBreakdown {
  채널: '매입DB' | '직접생산' | '현수막';   // 콜·지·기·소는 비용 0이라 도넛 제외
  비용: number;
}
...
GET  /api/dashboard                      → DashboardView (대시보드 탭 1회 batchGet 후 매핑)
