...
**시트 사전계산 vs React 동적계산**:
- 시트에 `[영업효율관리]` 영역(영업관리 G~H 1~6행)에 일부 사전 계산되어 있을 수 있음
- 4개 지표 정확히 위 공식과 일치하는지 디스커버리 후 확정. **현재는 React에서 동적 계산 권장**.
