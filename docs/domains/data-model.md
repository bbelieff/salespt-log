...
type Meeting = {
  ...
  // 미팅 완료 후 업데이트
  상태: '예약'|'완료'|'계약'|'변경'|'취소';  // J열 (5상태)
  계약여부: boolean;           // K열
  수임비: number;             // 만원 (L열)
  계약조건: string;           // M열 — 계약 시만 입력 (영업관리 P열의 "조건" 필드)

  // 수식 생성 (읽기 전용)
  표시상세: string;           // N열: "4/25 13:00 ○○치킨 방이동"
  표시요약: string;           // O열: "13:00 ○○치킨 방이동"

  // 일정·계약 탭의 [완료/변경/취소] 액션 시 prepend 누적 (계약은 안 적음)
  미팅결과: string;           // P열 — "[완료(계약X)] 업체명, 사유" 형식
                              // 영업관리 M열로 TEXTJOIN 자동 흐름
};
