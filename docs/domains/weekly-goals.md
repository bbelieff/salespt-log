...
미리보기에서 상단 입력이 없는 여섯 항목만 복사용 편집을 허용하며 원본 기록을 바꾸지 않는다. 목표/내부 원본 초안은 먼저 저장해야 복사 가능. HTML은 텍스트 escape와 br로 줄바꿈 보존; TSV는 내부 tab을 공백, 줄바꿈을 / 로 바꿔14열1행 유지. clipboard 실패/미지원이면 선택 가능한 text. 성공 문구는 복사됨뿐. 실제 Notion 표 붙여넣기 호환은 별도 NOT_RUN.
...
## 2026-09-14 편집·복사 정리
지난 PT과제와 성과는 현재 과제 위에서 행별로 함께 표시한다. 성과는 기존 priorOutcome 행 위치 저장과 내부 권한을 유지한다. 내부 권한 화면의 기본 복사 표·수동 복사·저장 후 자동 복사는 노션의 동일한 14열(지역부터 목표계약) 데이터 한 행을 사용한다. 공용 화면은 내부 정보를 제외한 표에서 인적 정보 다음 PT과제, 다섯 목표 순서를 사용한다. 회의록의 특이사항·지난 과제 성과·이번 과제·다섯 목표는 상단 초안에서 파생된 읽기 전용 값이다. 나머지 여섯 항목만 복사용으로 편집하며 저장 기록은 바꾸지 않는다. 미저장 상태에서는 복사를 차단한다.
