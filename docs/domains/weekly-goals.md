...
미리보기는 경영일지 조회값과 상단 초안을 자동 표시하며 직접 편집하지 않는다. 목표/내부 원본 초안은 먼저 저장해야 복사 가능. HTML은 텍스트 escape와 br로 줄바꿈 보존; TSV는 내부 tab을 공백, 줄바꿈을 / 로 바꿔14열1행 유지. clipboard 실패/미지원이면 선택 가능한 text. 성공 문구는 복사됨뿐. 실제 Notion 표 붙여넣기 호환은 별도 NOT_RUN.
...
자동 채움 정정: 회의록의 지역·기수·수강생·담당T는 서버에서 선택 수강생 정보와 배정 트레이너 이름을 조회한다. 금주미팅·금주계약은 선택 주차(금~목)의 기존 완료 미팅·순계약 실적 정의를 사용한다. 수동 복사용 덮어쓰기는 없으며 14열 전체가 조회값과 상단 초안에서 파생된다. 등록 정보가 없으면 기존 미기재 표시, 실제 집계 0건은 0으로 구분한다.
- 주차 내비게이션은 공통 `top-app-content` 오프셋을 사용해 모든 화면 폭에서 헤더·페이지 배너 아래에 밀착한다. 회의록 미리보기는 일반 문서 흐름 안에서 펼친다.
