...
- **대시보드 전면 `#VALUE!` (신규 기수 시트)** → ① 04 탭 grid < 45열(수식의 AO/이월 가드가
  grid 밖 참조 — "Array arguments to COUNTIFS…") ② 템플릿에 구세대 결함 수식(AW 참조,
  2026-06-18 사고 세대) 잔존. 수리 = **04 grid 45열 확장 후 [ID로 수식 설치] 재실행**
  (9기 2026-07-07 실측·#490). 쓰기 규칙: 레지스트리=RAW, 개인 시트 날짜 셀(O1/O2)=USER_ENTERED.
  근본 개선(후보): installFormulas 도입부에 ensureGridColumns(04,45) 내장 → 재발 원천 차단.
