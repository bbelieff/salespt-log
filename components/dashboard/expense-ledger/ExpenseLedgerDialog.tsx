...
export default function ExpenseLedgerDialog({ open, onClose }: Props) {
  ...
  const unclassifiedRefs = useMemo<ReclassifiableExpenseItem[]>(() => {
    ...
    for (const entry of unclassifiedLedger.data?.entries ?? []) {
      if (entry.categoryId !== unclassifiedId || entry.system || entry.readOnly) continue;
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-4 pc:px-6">
          ...
          {workspace === "view" && (
            <section id="expense-view-panel" role="tabpanel" className="space-y-3">
              ...
              <ExpenseLedgerTable data={ledgerData} loading={ledger.isLoading} error={ledger.error} retrying={ledger.isFetching} onRetry={() => { void ledger.refetch(); }} mode={view} onOpenRecord={() => setWorkspace("record")} />
            </section>
          )}
