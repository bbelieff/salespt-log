...
function Content({ student, date, week, trainer, returnTo }: { student: string; date: string; week: string; trainer: boolean; returnTo?: string }) {
  ...
  const back = goalReturnTarget(returnTo, trainer);
  return <main className="min-h-dvh bg-gray-50 p-4">
    <div className="mx-auto max-w-5xl space-y-4">
      <button onClick={() => nav(() => router.push(back.href))} className="min-h-11 px-3 text-sm">← {back.label}</button>
