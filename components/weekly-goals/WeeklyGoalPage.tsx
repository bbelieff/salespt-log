...
function Content({ student, date, week, trainer }: { student: string; date: string; week: string; trainer: boolean }) {
  ...
  return <main className="min-h-dvh bg-gray-50 p-4">
    <div className="mx-auto max-w-5xl space-y-4">
      ...
      {query.isError && <div role="alert">{query.error.message}<button onClick={() => void query.refetch()} className="ml-3 min-h-11 underline">다시 시도</button></div>}
      {query.isPending && <p role="status">목표를 불러오는 중…</p>}
      {query.data && !goalAccessDenied(query.error) && <WeeklyGoalEditor key={[query.data.student.email, query.data.student.cohort, query.data.student.courseStart, query.data.current.start, generation].join("|")}
