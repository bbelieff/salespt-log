...
export default function GoalInternalEditor({ view, onDirty, publicDirty }: {
  ...
  const load = async () => {
    ...
    catch (e) { if (goalAccessDenied(e)) { setSaved(null); setDraft(null); } setError((e as Error).message); }
  };
  useEffect(() => {
  ...
  async function save() {
    ...
    } catch (e) { if (goalAccessDenied(e)) { setSaved(null); setDraft(null); } setError((e as Error).message); throw e; }
  ...
  return <section className="space-y-4 rounded-2xl border border-gray-200 bg-gray-50 p-4 ph-no-capture">
    ...
    {error && <div role="alert" className="text-sm text-red-600">{error}
      <button type="button" className="ml-2 min-h-11 underline" onClick={() => {
