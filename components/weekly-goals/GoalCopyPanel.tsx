...
export default function GoalCopyPanel({ view, internal, dirty, autoCopyToken }: {
  ...
  const [overrides, setOverrides] = useState<(string | undefined)[] | null>(null);
  ...
  const base = internal ? meetingCells(view, internal) : null;
  const cells = base?.map((s, i) => (i < 6 ? (overrides?.[i] ?? s) : s)) ?? null;
  ...
  return <section className="space-y-3">
    <div className="flex flex-wrap gap-2">
      ...
      {internal && <button type="button" disabled={unavailable} className="min-h-11 rounded-xl border border-gray-300 px-4 text-sm disabled:opacity-50"
        onClick={() => { setOverrides(Array<string | undefined>(6).fill(undefined)); setPreviewOpen(true); setFallback(""); setStatus(""); }}>회의록 미리보기</button>}
    </div>
    ...
    {previewOpen && cells && internal && <div className="space-y-3 rounded-xl border border-gray-200 p-4">
      ...
      <p className="text-sm text-gray-500">PT과제·성과·특이사항·목표는 위 입력란에서 수정해 주세요. 나머지 항목은 여기서 수정할 수 있으며 복사본에만 반영됩니다.</p>
        ...
        <label key={label} className="block text-sm">{label}
          {i < 6
            ? <textarea aria-label={`회의록 ${label}`} className="mt-1 w-full rounded-lg border border-gray-300 p-2" value={cells[i]} rows={2}
              onChange={e => setOverrides(prev => { const next = prev ? [...prev] : Array<string | undefined>(6).fill(undefined); next[i] = e.target.value; return next; })} />
      ...
      <button type="button" onClick={() => { setPreviewOpen(false); setOverrides(null); }} className="ml-2 min-h-11 px-4 text-sm">닫기</button>
    </div>}
