...
export async function ensureCohortsTab(seedLabels: string[] = []): Promise<void> {
  ...
  // 시드 — 위에서 existing.length===0 확인됨(빈 탭)이라 항상 A2 부터 순서대로 쓴다.
  // values.append 대신 결정적 좌표(claim-append-columns 패턴) — 테이블 자동탐지 열밀림 방지.
...
export async function setCohortStatus(
  ...
  // 없으면 새 row — 결정적 좌표(claim-append-columns 패턴), values.append 열밀림 방지.
