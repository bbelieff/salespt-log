...
describe("isDbReadPilot / chooseDailySource (파일럿 게이트)", () => {
  ...
  it("비대상 기수(1기 등)는 DB 켜져 있어도 시트 경로 고정 (수용 기준 — 불변)", () => {
    expect(chooseDailySource("1", true)).toBe("sheet");
    expect(chooseDailySource("2", true)).toBe("sheet");
  });
