...
const cp = (o: Partial<ContractPayment> = {}): ContractPayment =>
  ({
    ...
    업체명: "가나상사",
    수임비: 1_000_000,
    수납1: { 진행기관: "", 진행상품: "", 진행률: "", 현황: "", 승인금액: 0, 수납액: 0, 수납일: "", 메모: "" },
    수납2: { 진행기관: "", 진행상품: "", 진행률: "", 현황: "", 승인금액: 0, 수납액: 0, 수납일: "", 메모: "" },
    수납3: { 진행기관: "", 진행상품: "", 진행률: "", 현황: "", 승인금액: 0, 수납액: 0, 수납일: "", 메모: "" },
    ...o,
  }) as ContractPayment;
...
describe("validContractDraft", () => {
  it("accepts ordinary recordkeeping edits", () => {
    expect(
      validContractDraft(cp({ 수임비: 0, 수납1: { 진행기관: "미소", 진행상품: "", 진행률: "20%", 현황: "진행", 승인금액: 3_000_000, 수납액: 1_000_000, 수납일: "2026-08-01", 메모: "" } })),
    ).toBe(true);
  });
  ...
  it("rejects negative amounts and malformed dates", () => {
    ...
    expect(
      validContractDraft(cp({ 수납1: { 진행기관: "", 진행상품: "", 진행률: "", 현황: "", 승인금액: 0, 수납액: -5, 수납일: "", 메모: "" } })),
    ).toBe(false);
    expect(
      validContractDraft(cp({ 수납2: { 진행기관: "", 진행상품: "", 진행률: "", 현황: "", 승인금액: 0, 수납액: 0, 수납일: "26.8.1", 메모: "" } })),
    ).toBe(false);
  });
});
