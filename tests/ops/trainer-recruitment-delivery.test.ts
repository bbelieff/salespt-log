...
import { buildArtifact, verifyArtifact, validateInputs, validateInventory, sha256, PAYLOAD } from "../../scripts/ops/trainer-recruitment-delivery.mjs";
import { EXPECTED_CHECKSUM, VERSION } from "../../scripts/ops/trainer-recruitment-migrate.mjs";
...
beforeAll(async () => {
  const fixture = await mkdtemp(join(tmpdir(), "trainer-delivery-fixture-"));
  artifact = join(fixture, "payload");
  report = await buildArtifact(root, artifact, inputs);
});

describe("trainer-recruitment immutable delivery (synthetic filesystem, no SSH/DB)", () => {
  it("forbids ledger repair and requires runtime comparison for execution", () => {
    expect(() => validateInputs({...inputs,repairHistoryAcl:"true",compareRuntime:"true"})).toThrow("REPAIR_FORBIDDEN");
    expect(() => validateInputs({...inputs,execute:"true"})).toThrow();
    expect(validateInputs({...inputs,execute:"true",compareRuntime:"true"})).toContain("/opt/salespt-migrations/");
  });
  ...
  it("ships only exact SQL plus hashes for unrelated pending; verifies every helper byte", async () => {
    ...
    expect(report.files["scripts/ops/trainer-recruitment-migrate.mjs"]).toBe(sha256(await readFile(resolve(root, "scripts/ops/trainer-recruitment-migrate.mjs"))));
    expect(result.files.filter((f) => f.version.startsWith("0002_")).length).toBeGreaterThan(1);
  });
  it("has a self-contained executable module graph and no repair mode", async () => {
    const module = await import(/* @vite-ignore */ ("file://"+artifact+"/scripts/ops/trainer-recruitment-delivery-run.mjs"));
    expect(await module.runDelivery(["verify",inputs.sha,report.manifestChecksum,EXPECTED_CHECKSUM])).toMatchObject({mode:"ARTIFACT_VERIFIED"});
    await expect(module.runDelivery(["repair-history-acl",inputs.sha,report.manifestChecksum,EXPECTED_CHECKSUM])).rejects.toThrow("ARGUMENTS_INVALID");
  });
  ...
  it("rejects helper tampering and an extra unapproved SQL file", async () => {
    const fixture = await mkdtemp(join(tmpdir(), "trainer-delivery-tamper-"));
    const path = join(fixture, "payload");
    const made = await buildArtifact(root, path, inputs);
    await writeFile(join(path, "scripts/ops/trainer-recruitment-migrate.mjs"), "// altered fixture");
...
describe("trainer-recruitment protected delivery workflow contract", () => {
  ...
  it("verifies archive before extraction, all payload before ready, preflight before execute", async () => {
    const ssh = await readFile(resolve(root, "scripts/ops/trainer-recruitment-delivery-ssh.sh"), "utf8");
