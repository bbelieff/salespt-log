...
async function ensureSeasonRow() {
  ...
  if (idx < 0) {
    // 결정적 좌표(claim-append-columns 패턴) — values.append 테이블 자동탐지가
    // A2 registry 를 S~AL 로 밀어넣은 사고(2026-08-05)와 같은 클래스라 cohorts 도 동일 수정.
...
async function main() {
  ...
  // (구) --diag-append 진단 플래그 제거(BBE-97) — 2026-08-05 사고 원인 규명용
  // 1회성 코드였고, 실행할 때마다 운영 registry 에 ZZTEST 마커 행을 실제로 쓰는
  // values.append 호출을 포함해 원인이 이미 규명된 지금은 순수 리스크만 남았다.
  // (진단 로직 자체가 사고와 같은 API 를 그대로 재현하려던 것이라 "격리"보다 제거가 맞다.)
