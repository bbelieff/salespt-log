...
MANIFEST_HASH=$(sha256sum "$RUNNER_TEMP/weekly-payload/manifest.json" | cut -d' ' -f1)
...
ssh_retry "set -eu; umask 077
  ...
  node '$STAGE/payload/scripts/ops/weekly-goals-delivery-run.mjs' verify '$EXPECTED_SHA' '$MANIFEST_HASH' '$EXPECTED_SQL_SHA256'
