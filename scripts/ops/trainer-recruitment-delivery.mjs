#!/usr/bin/env node
// #956 immutable migration-only artifact. No connection or app checkout mutation here.
...
import { VERSION, EXPECTED_CHECKSUM } from "./trainer-recruitment-migrate.mjs";

export const PAYLOAD = ["scripts/db-migrate.mjs", "scripts/ops/trainer-recruitment-migrate.mjs",
  "scripts/ops/trainer-recruitment-migrate-catalog.mjs", "scripts/ops/weekly-goals-migrate-catalog.mjs",
  "scripts/ops/weekly-goals-runtime.mjs", "scripts/ops/weekly-goals-migrate.mjs",
  "scripts/ops/trainer-recruitment-delivery.mjs", "scripts/ops/trainer-recruitment-delivery-run.mjs",
  `lib/repo/db/migrations/${VERSION}`, "inventory.json", "runtime-contract.json"];
...
export function validateInputs({ sha, actualSha, sqlChecksum, mode, execute, runId, attempt, repairHistoryAcl = "false", compareRuntime = "false" }) {
  ...
  if (repairHistoryAcl !== "false") fail("REPAIR_FORBIDDEN");
